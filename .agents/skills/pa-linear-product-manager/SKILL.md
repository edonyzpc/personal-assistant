...
description: Manage Personal Assistant product intake, version planning, milestone rhythm, Linear issue hygiene, Chinese-first PA planning content, and product/milestone-level release-readiness workflows with minimal user management load. Use when the user says things like "记录一个 PA idea", "整理需要我决策的 PA 项", "帮我排 vNext 候选", "PA 当前版本状态怎么样", "检查 PA 现在能不能发版", asks to route ideas/features through Linear, or wants the agent to handle PA version-management chores while they focus on product, design, and business decisions. For code-level release-readiness review (diff, tests, compatibility), use `personal-assistant-review` instead.
---
...
- Repository docs are the design and evidence system. Use current docs such as `docs/todo.md`, `docs/development-roadmap.md`, `docs/release-process.md`, active SDDs, and active trackers when planning. Historical trackers are in `docs/archive/`.
