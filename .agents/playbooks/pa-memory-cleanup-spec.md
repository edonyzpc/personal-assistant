...
3. **Memory Codex 可达性** → 双写：Claude memory 原位保留 + 导出 `docs/product/active-decisions.md` 到 repo
...
### Step 7: 生成 `docs/product/active-decisions.md`

从全部 memory 文件中提取**仍然活跃**的决策，写入 `docs/product/active-decisions.md`。
...
- [ ] `docs/product/active-decisions.md` 存在且覆盖所有非 `[DONE]` memory 的关键决策
...
- `docs/product/active-decisions.md` 每条决策标注 `source:` 文件名
