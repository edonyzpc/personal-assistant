...
describe("B-125 iOS WKWebView runtime receipt", () => {
    ...
    it("prints a Safari-compatible async-IIFE capture command", () => {
        const directory = createTemporaryDirectory();
        const bundlePath = join(directory, "fts-ios-runtime-bundle.js");
        const result = spawnSync(process.execPath, [
            preparePath,
            "--output", bundlePath,
            "--plugin-artifact", currentDistPluginArtifactPath,
            "--vault-path", bundleVaultPath,
        ], { encoding: "utf8" });

        expect(result.status).toBe(0);
        const consoleBlock = result.stdout.match(/```js\n([\s\S]*?)\n```/u)?.[1];
        expect(consoleBlock).toBeDefined();
        expect(() => new Script(consoleBlock!)).not.toThrow();
        expect(consoleBlock).toContain("(async () => {");
        expect(consoleBlock).toContain("await (0, eval)(paFtsIosBundleSource)");
        expect(consoleBlock).toContain("})()");
    });
