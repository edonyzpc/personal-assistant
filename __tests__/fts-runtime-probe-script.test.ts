...
describe("portable exact Obsidian FTS runtime evidence", () => {
    ...
    it("accepts the isolation-bound target-id CLI contract without weakening fail-closed capture", () => {
        const result = spawnSync(
            process.execPath,
            [probePath, "--target-id=isolated-loaded-app", "--json"],
            { encoding: "utf8" },
        );
        expect(result.status).toBe(2);
        ...
        expect(JSON.parse(result.stdout)).toMatchObject({
            ...
            exactRenderer: null,
            evaluation: {
                blockers: expect.arrayContaining(["exact_obsidian_renderer_missing"]),
            },
        });

        const empty = spawnSync(
            process.execPath,
            [probePath, "--target-id=", "--json"],
            { encoding: "utf8" },
        );
        expect(empty.status).toBe(1);
        expect(empty.stderr).toContain("--target-id requires a non-empty CDP target id");
    });
    ...
    it("verifies loaded-app and shell identities independently", () => {
        ...
        }, expectedShell)).toMatchObject({
            exactRendererIdentity: false,
            identityVerified: false,
        });
    });

    it("selects the loaded app renderer when starter is listed first", () => {
        const starter = {
            id: "starter",
            type: "page",
            title: "Obsidian",
            url: "app://obsidian.md/starter.html",
            webSocketDebuggerUrl: "ws://127.0.0.1/starter",
        };
        const loadedApp = {
            id: "loaded-app",
            type: "page",
            ...
            webSocketDebuggerUrl: "ws://127.0.0.1/index",
        };

        expect(selectLoadedAppTarget([starter, loadedApp])).toEqual(loadedApp);
    });

    it("fails closed when more than one exact loaded app renderer is present", () => {
        const first = {
            id: "loaded-app-a",
            type: "page",
            url: "app://obsidian.md/index.html",
            webSocketDebuggerUrl: "ws://127.0.0.1/index-a",
        };
        const second = {
            id: "loaded-app-b",
            type: "page",
            url: "app://obsidian.md/index.html",
            webSocketDebuggerUrl: "ws://127.0.0.1/index-b",
        };

        const forward = selectLoadedAppTargetError([first, second]);
        const reverse = selectLoadedAppTargetError([second, first]);
        expect(forward).toBe(reverse);
        expect(forward).toContain("refusing ambiguous renderer selection");
    });

    it("selects only the exact isolation-bound loaded renderer when target-id is supplied", () => {
        const first = {
            id: "loaded-app-a",
            type: "page",
            url: "app://obsidian.md/index.html",
        ...
        expect(selectLoadedAppTarget([second], second.id)).toEqual(second);
        expect(selectLoadedAppTargetError([first], "missing")).toContain(
            "is not the unique exact app://obsidian.md/index.html page target",
        );
        expect(selectLoadedAppTargetError([starterWithBoundId], starterWithBoundId.id)).toContain(
            "no exact app://obsidian.md/index.html page target",
        );
        expect(selectLoadedAppTargetError([first, second], second.id)).toContain(
            "refusing ambiguous renderer selection",
        );
    });
});
...
function makePlatformReceipt(base: any, platform: string): any {
    ...
    receipt.exactRenderer = {
        ...
        profile,
        target: { title: "Obsidian", url: "app://obsidian.md/index.html" },
        pluginIdentity: {
        ...
        runtime: {
            ...
            browser: {
                ...
                platform,
                language: "en-US",
                locationHref: "app://obsidian.md/index.html",
            },
        },
    };
...
function selectLoadedAppTarget(
    targets: readonly Record<string, unknown>[],
    targetId: string | null = null,
): Record<string, unknown> {
    const result = runTargetSelector(targets, targetId);
    if (result.status !== 0) throw new Error(result.stderr);
    return JSON.parse(result.stdout) as Record<string, unknown>;
}

function selectLoadedAppTargetError(
    targets: readonly Record<string, unknown>[],
    targetId: string | null = null,
): string {
    const result = runTargetSelector(targets, targetId);
    if (result.status === 0) throw new Error("Expected loaded app target selection to fail.");
...
function runTargetSelector(
    ...
    const moduleUrl = pathToFileURL(probePath).href;
    const source = [
        `import { selectExactObsidianLoadedAppTarget } from ${JSON.stringify(moduleUrl)};`,
        "try {",
        `  const result = selectExactObsidianLoadedAppTarget(${JSON.stringify(targets)}, ${JSON.stringify(targetId)});`,
        "  process.stdout.write(JSON.stringify(result));",
        "} catch (error) {",
