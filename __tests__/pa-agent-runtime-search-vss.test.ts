...
describe("MemorySearchTool searchVss contract", () => {
    ...
    it("passes the rewrite promise to searchHybrid without awaiting it (true parallel path)", async () => {
        ...
        let resolveRewrite: ((v: string | null) => void) | null = null;
        const rewriteDeferred = new Promise<string | null>((r) => { resolveRewrite = r; });
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (tool as any).rewriteQueryWithTimeout = jest.fn(async () => rewriteDeferred);
