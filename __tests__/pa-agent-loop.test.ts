...
describe("PaAgentLoop", () => {
    ...
    it("lets one dispatched buffered request finish after softAt without starting tools or another turn", async () => {
        ...
        try {
            ...
            const execute = jest.fn(async () => ({
                outcome: "success" as const,
                promptText: "unexpected",
            }));
            const hostPolicy = jest.fn(() => ({
                action: "continue" as const,
                reason: "corrective_turn" as const,
            }));
            const reserveEvents: Array<{ stage: string; remainingMs: number }> = [];
            ...
            const loop = new PaAgentLoop({
                runId: "run-buffered-soft-overrun",
                userInput: "discover",
                model: {
                    stream: async function* (input) {
                        modelInputs.push(input);
            ...
            expect(result.committedFinalText).toBe("Buffered final answer.");
    ...
    it("rejects buffered tool continuation after softAt without executing or finalizing with a second request", async () => {
        jest.useFakeTimers();
        try {
            const modelInputs: PaAgentModelInput[] = [];
            const execute = jest.fn(async () => ({
    ...
    it("keeps a dispatched buffered request alive only until hardAt and never overlaps it", async () => {
        jest.useFakeTimers();
        try {
            const modelInputs: PaAgentModelInput[] = [];
            let providerSignal: AbortSignal | undefined;
            const loop = new PaAgentLoop({
                runId: "run-buffered-hard-deadline",
                userInput: "discover",
                model: {
                    stream: async function* (input) {
                        modelInputs.push(input);
    ...
    it("does not dispatch invoke when buffered fallback preparation crosses softAt", async () => {
        jest.useFakeTimers();
        try {
            const stream = jest.fn(async () => {
                throw new Error("stream setup failed");
            });
            const invoke = jest.fn(async () => ({ content: "unexpected" }));
            const modelInputs: PaAgentModelInput[] = [];
            const loop = new PaAgentLoop({
                runId: "run-buffered-fallback-soft-admission",
                userInput: "discover",
                model: {
                    stream: async function* (input) {
                        modelInputs.push(input);
                        if (input.toolMode === "final_answer_only") {
                            yield { type: "text_delta", text: "Safe final answer." } as const;
                            return;
                        }
                        yield* streamWithInvokeFallback({
                            chain: {
                                stream: async (_chainInput: unknown, _options?: { signal?: AbortSignal }) => {
                                    input.notifyProviderRequestStarted?.();
                                    return stream();
                                },
                                invoke: async (_chainInput: unknown, _options?: { signal?: AbortSignal }) => {
                                    input.notifyProviderRequestStarted?.();
                                    return invoke();
                                },
                            },
            ...
            expect(result.status).toBe("completed");
            expect(result.committedFinalText).toBe("Safe final answer.");
        } finally {
            jest.useRealTimers();
        }
    });
    ...
    it("executes multiple toolcalls in assistant index order even when deltas arrive out of order", async () => {
        ...
        expect(result.transcript.find((message) => message.role === "assistant")?.content)
            .toEqual(result.turns[0].assistantMessage.content);
    });
    ...
    it("turns tool-specific timeout into the configured tool outcome", async () => {
        ...
        expect(outerToolDeadlineAt).toBe(101);
        expect(events.find((event) => event.type === "tool_execution_end")).toMatchObject({
            outcome: "policy_rejected",
        });
        expect(result.turns[0].toolResults[0]).toMatchObject({
