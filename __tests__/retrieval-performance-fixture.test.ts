...
interface RetrievalFixtureManifest {
    ...
    compactProxyPlan: {
        ...
        requiredPlatformClass: string;
        requiredRuntimeFamily: string;
        deviceIdentityPolicy: string;
        settingsPhases: {
            control: Record<string, boolean>;
            evaluated: Record<string, boolean>;
            requiredTransitionCount: number;
        };
        hardBudgets: Record<string, number>;
        workload: {
            schemaVersion: number;
            fixtureCaseId: string;
            prompts: Record<string, { text: string }>;
            stages: Record<string, Array<StageSegment & { settingsPhase: string }>>;
        };
        ...
        requiredResourceMetrics: string[];
        optionalResourceMetrics: string[];
        independentCorrectnessSlices: string[];
    };
...
describe("retrieval performance fixture", () => {
    it("adds an independent compact proxy without changing strict v9 topology", () => {
        const plan = manifest.compactProxyPlan;
        const control = expandStage(plan.workload.stages.controlStandard);
        const standard = expandStage(plan.workload.stages.evaluatedStandard);
        const retry = expandStage(plan.workload.stages.evaluatedRetry);
        const cancellation = expandStage(plan.workload.stages.cancellationProbe);
        const sequence = [...control, ...standard, ...retry, ...cancellation];

        expect(plan).toMatchObject({
            schemaVersion: 1,
            version: "b125-compact-proxy-v1",
            ...
            deviceIdentityPolicy: "operator-provided-sha256",
            settingsPhases: {
                control: {
            ...
            workload: {
                schemaVersion: 1,
                fixtureCaseId: workload.fixtureCase.id,
                prompts: {
                    "standard-v1": { text: workload.prompts["standard-v1"].text },
                    "retry-v1": { text: workload.prompts["retry-v1"].text },
                    "cancel-v1": { text: workload.prompts["cancel-v1"].text },
                },
            },
            requiredMaintenance: ["rebuild", "incremental-update"],
            maintenanceOperations: {
                rebuild: {
                    sequence: 1,
                    kind: "rebuild",
                ...
                incrementalUpdate: {
                    ...
                    inputSource: "indexed-chunks",
                    fixturePath: "retrieval-smoke/lexical/量子灯塔检索.md",
                },
            },
            ...
            optionalResourceMetrics: [
                "memory.peakProcessFootprintBytes",
                "memory.heapUsedBytes",
            ],
            independentCorrectnessSlices: [
                "six-ranking-cases",
                "structured-temporal-retry",
            ],
        });
        expect([control.length, standard.length, retry.length, cancellation.length])
            .toEqual([6, 13, 13, 1]);
        expect(sequence).toHaveLength(33);
        expect(new Set(sequence.map(({ id }) => id)).size).toBe(33);
        expect(control.map(({ sampleClass }) => sampleClass)).toEqual([
            "warmup", ...Array(5).fill("measured"),
        ]);
        expect(standard.map(({ sampleClass }) => sampleClass)).toEqual([
            ...Array(3).fill("warmup"), ...Array(10).fill("measured"),
        ]);
        expect(retry.map(({ sampleClass }) => sampleClass)).toEqual([
            ...Array(3).fill("warmup"), ...Array(10).fill("measured"),
        ]);
        expect(plan.workload.stages.controlStandard
            .every(({ settingsPhase }) => settingsPhase === "control")).toBe(true);
        expect([
            ...plan.workload.stages.evaluatedStandard,
            ...plan.workload.stages.evaluatedRetry,
            ...plan.workload.stages.cancellationProbe,
        ].every(({ settingsPhase }) => settingsPhase === "evaluated")).toBe(true);

        expect(manifest.deviceMeasurementPlan.version).toBe("b125-device-measurement-v9");
        expect([
            expandStage(workload.stages.standardPerformance).length,
            expandStage(workload.stages.retryPerformanceBatch1).length,
            expandStage(workload.stages.retryPerformanceBatch2).length,
            expandStage(workload.stages.cancellationProbe).length,
        ]).toEqual([23, 12, 11, 1]);
    });
