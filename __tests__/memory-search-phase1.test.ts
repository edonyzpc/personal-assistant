...
describe("Memory search readiness cancellation", () => {
    it("passes the active tool signal into Memory readiness", async () => {
        ...
        await tool.search("question", controller.signal);

        expect(ensureReadyForChat).toHaveBeenCalledWith(
            "question",
            controller.signal,
            controller.signal,
        );
    ...
    it("keeps the attempt signal separate from the detached preparation owner", async () => {
        const ensureReadyForChat = jest.fn(async (
            _query?: string,
            _signal?: AbortSignal,
            _ownerSignal?: AbortSignal,
        ...
        const invocation = createStandardMemorySearchInvocation({
            ...
            memoryPreparationOwnerSignal: owner.signal,
        });

        await runWithMemorySearchInvocation(
            invocation,
            attempt.signal,
            () => tool.search("question", attempt.signal),
        );
...
describe("Phase 1 reranker invocation", () => {
    it("rewrites and reranks when AbortSignal.any is unavailable", async () => {
        ...
        const removeListener = jest.spyOn(parent.signal, "removeEventListener");
        const llm = RunnableLambda.from(async (input: unknown) => {
            const promptValue = input as { toChatMessages?: () => Array<{ content?: unknown }> };
            const system = String(promptValue.toChatMessages?.()[0]?.content ?? "");
            return {
                content: system.includes("ranking")
                    ? '{"verdict":"relevant","ranking":[0],"needsMoreEvidence":false}'
    ...
    it("reranks even one candidate with the selected Chat model", async () => {
        ...
        const promptValue = invoke.mock.calls[0]?.[0] as {
            toChatMessages?: () => Array<{ content?: unknown }>;
        };
        const systemPrompt = promptValue.toChatMessages?.()[0]?.content;
        expect(systemPrompt).toEqual(expect.stringContaining(
            "Judge answer sufficiency from the ranked evidence as a whole",
        ));
        expect(systemPrompt).toEqual(expect.stringContaining(
            "leaves a requested fact or conclusion unresolved",
        ));
        expect(systemPrompt).toEqual(expect.stringContaining(
            "ranked evidence as a whole still leaves it unknown, unconfirmed, missing, or incomplete",
        ));
        expect(systemPrompt).toEqual(expect.stringContaining(
            "other ranked evidence answers the exact request",
        ));
        expect(systemPrompt).toEqual(expect.stringContaining(
            'exactly the keys "verdict", "ranking", and "needsMoreEvidence"',
        ));
...
describe("Phase 1 search assembly", () => {
    async function runSearch(options: {
        ...
        const invocation = options.captureRecoverySeed
            ...
            || options.invocationOrdinal !== undefined
            || Number.isFinite(options.absoluteDeadlineMs)
            || options.providerRequestScope !== undefined
            ? createStandardMemorySearchInvocation({
    ...
    it("propagates the run scope to the strict reranker model", async () => {
        ...
        expect(search.createChatModel).toHaveBeenCalledWith(0, expect.objectContaining({
            modelName: "chat-model",
            providerRequestScope,
        }));
        expect(search.createChatModel).not.toHaveBeenCalledWith(
            expect.anything(),
    ...
    it("binds the invocation ordinal to Memory, graph, and reranker diagnostics", async () => {
        ...
        expect(diagnostics).toEqual(expect.arrayContaining([
            expect.objectContaining({ phase: "memory_search", invocationOrdinal: 4 }),
            expect.objectContaining({ phase: "graph_snapshot", invocationOrdinal: 4 }),
            expect.objectContaining({ phase: "reranker", invocationOrdinal: 4 }),
        ]));
        expect(diagnostics.every((event) => event.invocationOrdinal === 4)).toBe(true);
    });
