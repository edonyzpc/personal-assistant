...
describe("RetrievalDiagnosticsController", () => {
    ...
    test("allow-lists only non-negative safe invocation ordinals", () => {
        const controller = new RetrievalDiagnosticsController(
            () => 0,
            () => 0,
            () => "session-invocation-ordinal",
        );
        const session = controller.start();
        for (const invocationOrdinal of [0, Number.MAX_SAFE_INTEGER, -1, 0.5, Number.MAX_SAFE_INTEGER + 1]) {
            controller.record("chat", {
                invocationOrdinal,
                phase: "memory_search",
                outcome: "completed",
        ...
        expect(controller.snapshot(session.sessionId).events).toEqual([
            expect.objectContaining({ invocationOrdinal: 0 }),
            expect.objectContaining({ invocationOrdinal: Number.MAX_SAFE_INTEGER }),
            expect.not.objectContaining({ invocationOrdinal: expect.anything() }),
            expect.not.objectContaining({ invocationOrdinal: expect.anything() }),
            expect.not.objectContaining({ invocationOrdinal: expect.anything() }),
            expect.not.objectContaining({ invocationOrdinal: expect.anything() }),
            expect.objectContaining({ invocationOrdinal: 8 }),
        ]);
    });

    test("preserves configured reserve binding and reserve-overrun terminals", () => {
        const controller = new RetrievalDiagnosticsController(
            () => 0,
            () => 0,
            () => "session-finalization-reserve",
        );
        const session = controller.start();
        controller.record("chat", {
            runId: "run-finalization-reserve",
            phase: "finalization_reserve",
            outcome: "deadline",
            reason: "reserve_overrun",
            metrics: { configuredReserveMs: 10_000, remainingMs: 9_000 },
        });
...
describe("retrieval diagnostic outcome classification", () => {
    ...
    test("suppresses only the duplicate cooperative-abort terminal", () => {
        expect(shouldRecordGraphWorkerFailureTerminal("graph-rank-aborted", 1, false)).toBe(false);
        expect(shouldRecordGraphWorkerFailureTerminal("aborted", 1, false)).toBe(false);
        expect(shouldRecordGraphWorkerFailureTerminal("graph-rank-aborted", 0, false)).toBe(true);
        expect(shouldRecordGraphWorkerFailureTerminal("aborted", 1, true)).toBe(true);
        expect(shouldRecordGraphWorkerFailureTerminal("graph-rank-deadline", 1, false)).toBe(true);
        expect(shouldRecordGraphWorkerFailureTerminal("graph-rank-worker-error", 1, false)).toBe(true);
    });
});
