...
const OBSIDIAN_LOADED_APP_URL = "app://obsidian.md/index.html";

function parseArguments(argv) {
  const options = { format: "markdown", cdp: null, targetId: null };
  for (const argument of argv) {
    ...
    else if (argument.startsWith("--target-id=")) {
      const targetId = argument.slice("--target-id=".length);
      if (!targetId) throw new Error("--target-id requires a non-empty CDP target id.");
...
function isExactObsidianLoadedAppTarget(target) {
  return target.type === "page"
    && target.webSocketDebuggerUrl
    && target.url === OBSIDIAN_LOADED_APP_URL;
}

export function selectExactObsidianLoadedAppTarget(targets, targetId = null) {
  if (!Array.isArray(targets)) {
    throw new Error("CDP target list is not an array.");
  }
  const candidates = targets.filter(isExactObsidianLoadedAppTarget);
  if (candidates.length === 0) {
    throw new Error(`CDP has no exact ${OBSIDIAN_LOADED_APP_URL} page target.`);
  }
  if (candidates.length !== 1) {
    throw new Error(`CDP has ${candidates.length} exact ${OBSIDIAN_LOADED_APP_URL} page targets; refusing ambiguous renderer selection.`);
  }
  if (targetId !== null && candidates[0].id !== targetId) {
    throw new Error(
      `CDP target ${targetId} is not the unique exact ${OBSIDIAN_LOADED_APP_URL} page target.`,
    );
  }
  return candidates[0];
}
...
export function inspectCdpRuntimeIdentity(runtime, expectedIdentity) {
  ...
  const exactRendererIdentity = runtime?.host === "electron-renderer"
    ...
    && runtime?.browser?.locationHref === OBSIDIAN_LOADED_APP_URL
...
async function evaluateCdp(endpoint, source, profileSource, expectedIdentity, targetId = null) {
  let targets;
  let lastError;
  ...
  const target = selectExactObsidianLoadedAppTarget(targets, targetId);
