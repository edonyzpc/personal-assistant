...
| SPEC-D5 | Context Compactor (micro + full compaction) | v2.5 | `[x]` Implemented |
| SPEC-D6 | Context Budget (chars/tokens 追踪 + provider usage 回写) | v2.5 | `[x]` Implemented |
...
- D5/D6 are implemented under `src/ai-services/context/`; provider token usage is recorded as diagnostics and fed back into the budget tracker when provider chunks expose usage, with char/token estimates always available.
- D7/D8 are implemented under `src/ai-services/memory-extraction/`: Type A uses local explicit-preference/correction extraction plus recurrence promotion and IndexedDB storage; Type C uses Obsidian metadata/topology analysis as an internal snapshot and does not write vault notes or inject prompt context by default.
...
| 注入方式 | always-loaded | 默认不注入；future on-demand |
