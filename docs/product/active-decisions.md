...
| DEC-014 | Operations Agent Step 2 仅开放有界 Chat → vault 写入 | build gate 可用但 per-vault 设置默认关闭；仅四个 core tools、单 intent inline 确认、stale-safe 执行、drift-safe Undo 与 content-free audit；Step 3 Pagelet integration 及额外写入仍关闭 | [Decision Record](./decisions/dec-014-defer-operations-agent.md), [Step 2 SDD](../development/proposals/operations-agent/operations-agent-step2-sdd.md), [Backlog B-101](../backlog.md) | 真实 dogfood/安全证据要求缩窄，或 owner 评估 Step 3 / 额外写入 |
