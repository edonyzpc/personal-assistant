...
2026-09-12 D16 补充决定：Owner明确macOS/iOS真机成本高，只用于与平台本身强相关的改动；平台无关行为复用同一冻结输入的Linux/Desktop证据，移动呈现优先使用Obsidian CLI mobile simulator。B-135须把每个设备门映射到实际平台分支或系统能力；只有Keychain、iCloud/文件提供器、HEIC、真实触控/软键盘、WKWebView专属生命周期或其它明确平台风险实际受影响时才保留真机门。B-135改动面审计未发现上述专属实现，390×844 simulator已补充设置和恢复模态证据，因此旧笼统“受影响iOS”不再阻塞T-11/T-18/T-21。simulator不证明真机专属事实，本决定也不取消其它功能自身的设备门。
