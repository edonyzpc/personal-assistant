...
`@CreateImage` 候选在当前输入位置附近弹出，随换行和滚动定位，并避让窄屏边界；
Enter 选择候选，Escape 关闭，中文输入法确认不误发送。

生成卡片逐步显示真实状态。每个输出图保持独立操作入口；失败项单独说明，已成功项
仍可使用。后续编辑生成新结果；“重新生成”与“编辑指定图片”保留不同的输入语义。
生成结果尚未可用时，Agent 不声称已看见或可以准确描述其内容。

图片融入所属助手消息，完整按比例展示。图内常驻编辑和下载入口，复制、保存到笔记
及重新生成与 Chat 消息按钮风格一致，详情置于图片之后折叠展示；多图操作各自绑定
确切输出，任务完成更新不重排已有消息或结果。
