...
- **B-129/REQ-01 — 首版范围与入口。** 在现有 Chat 提供图片理解、个性化文案和
  ...
  或进入文案生成流程。
  2026-09-08 用户确认收敛入口：输入框旁的图片按钮统一打开来源选择，桌面提供
  从文件、从笔记库添加，移动端另提供选择照片。文件、拖入、相册、粘贴共用
  实际交付文件的导入语义；不因来源提示“原件未验证”或要求补充相机原件。
  Chat 更多菜单不重复放置添加入口；来源选择方式不代表不同的原图保留承诺。
...
- **B-129/REQ-04 — 实际交付文件保留与追溯。** 原图指用户实际交给 PA 的受支持
  图片文件，不要求证明为相机原件。添加、拖入、相册、粘贴均完整保留收到的字节。
  导入成功即保存于 Obsidian 附件目录下的 `pa-images` 子目录，不必等到发送；
  原图不因生成处理副本而被覆盖。成为笔记附件时按 REQ-11 移动同一文件。
  已有 vault 图片优先记录引用，避免重复导入。引用应可追溯到原文件及内容版本；
  重命名、移动、删除或内容替换需正确识别，不能把路径相同的新内容当作旧图。
  系统已交付 JPEG 时按普通 JPEG 接受，不要求追溯其 HEIC 来源；HEIC 输入边界见 REQ-15。
- **B-129/REQ-05 — 本地副本与清理。** 聊天显示轻量副本，处理缓存有总容量
  ...
  2026-09-08 用户确认原图管理移至设置的“功能 → 聊天图片”，不再常驻 Chat
  更多菜单；首次使用说明中的再次查看路径同步更新。
  从草稿移除不自动删除已经导入的文件。迁出 `pa-images` 成为正式附件后，应退出
  聊天专用原图清理范围；其他聊天或笔记仍可引用它，不能因曾由 PA 导入继续取得删除权限。
- **B-129/REQ-06 — 三类同步边界。** 原图子目录默认目标是排除 Obsidian Sync、
  ...
  `pa-images` 仅存放聊天专用文件。PA 保存时迁出的完整文件按普通附件同步规则
  处理，原子目录排除不再适用；预览和说明需表达该变化，不改动整个普通附件目录的同步设置。
...
- **B-129/REQ-11 — PA 保存触发附件迁移与复用。** 仅在用户通过 PA 确认执行
  “保存为笔记”时，将本次选中且位于聊天专用 `pa-images` 的图片移至按目标
  笔记解析的普通附件目录，完整字节不变，同时更新聊天引用与笔记嵌入链接。
  例如 `attachments/pa-images/photo.jpg` 移至 `attachments/photo.jpg`；
  `attachments` 只是示例，仍使用 Obsidian 的附件目录和链接规则，不硬编码路径。
  已在普通 vault 位置的图片直接引用；首次迁出后其他聊天、其他笔记及再次保存
  共用该文件，不复制回 `pa-images`，不随每次保存重复复制或搬动。
  未选中图片不移动；生成、打开预览、取消预览、手动在笔记引用以及其他插件的
  引用行为均不触发移动，不新增全库引用监听搬迁功能。
  正式附件独立于可清理缓存，遵循正常同步设置。延续先登记保存记录、创建正文，
  再补齐附件与链接的可恢复保存原则；移动前后须记录并核对结果，处理同名冲突、
  多聊天共享、并发保存和用户修改，不覆盖已有文件，不使旧聊天失去图片，不留下
  无法解释的部分结果。重试复用已经核验的移动/链接结果，不生成另一份附件。
  2026-09-08 用户确认保留保存结果旁的核对/继续动作；更多菜单仅在确有未完成
  保存记录时展示汇总入口，每次打开重新查询，完成后不再显示空的待处理入口。
...
- **B-129/REQ-15 — 跨设备处理质量与失败恢复。** 输入格式、照片方向、小字
  ...
  2026-09-08 用户明确取消 HEIC 支持：HEIC 需由用户或系统在交付 PA 前转换为
  JPEG；PA 只保存实际收到的 JPEG。新的 HEIC 输入应在登记/写入聊天附件前拒绝，
  提示先转 JPEG；不调用设备解码、系统转换器或远程转换作为 HEIC 兜底，不通过
  改扩展名或 MIME 绕过格式识别。文件、相册、粘贴、拖入及库内选择遵循同一边界。
  这不意味着其他受支持格式都必须由用户转 JPEG，也不授权删除或批量转换已有 HEIC。
...
- NG-06: 首版不交付完整动画理解或依赖外部资源的 SVG 渲染；原文件保留及
  提供静态图的恢复入口仍属本版要求。
- NG-07: PA 不接收新的 HEIC 聊天附件，不提供内部 HEIC 转换或原 HEIC/JPEG
  双份保留功能；不因手动笔记引用自动迁移文件，不自动清理历史重复图片。
...
3. 在写入前识别并拒绝 HEIC，提示先转 JPEG，保留已有文字和其他有效图片。
   受支持文件可追溯地保留后生成预览与发送副本；逐项展示处理中、可发送或失败状态。
...
flowchart TD
  ...
  B --> C[预览文字、图片与目标位置]
  C --> D[确认执行：迁移聊天专用图片或引用已有附件]
  D --> J[更新聊天定位和笔记链接，核对完成]
  A --> E[以后参考这种风格]
  E --> F[记录文字版本、来源和类似场景]
...
| 新添加的文件实际为 HEIC | 不写入聊天附件、不内部转换，提示先转 JPEG；不影响当前其他有效输入 |
...
| 数据 | 位置与用途 | 清理与同步边界 |
| --- | --- | --- |
| 聊天专用图片 | `pa-images` 中完整保留的实际交付文件 | 可单独排除同步；移除草稿/删除聊天不自动删文件，PA 保存选中图片时迁出 |
| 预览与发送副本 | 本设备有容量上限的可重建缓存 | 可清理；去除敏感元数据的副本才可发给 provider |
| 聊天中的图片引用 | 随本设备聊天历史追溯图片身份、版本和顺序 | 不携带原图二进制；不由 vault 图片存在推导跨设备续聊 |
| 正式笔记和普通 vault 附件 | 用户明确保存的文字版本；从聊天目录迁出的文件或直接引用的已有图片，聊天与笔记共用 | 遵循正常附件同步配置；退出聊天专用清理范围，删除共享文件可能影响多个引用 |
| 风格参考 | 用户明确指定的文字版本、来源、场景和授权 | 可查看、取消；不随保存自动产生，不扩大既有 Memory 权限 |

PA 收到的文件可能保留拍摄位置等元数据，迁出成为正式附件后仍保留这些信息，
并可能随普通附件同步。只有 provider 输入处理副本按 REQ-07 清除源敏感元数据，
不能宣称用户或系统预先转换的 JPEG 已经去除元数据；去元数据不遮住图片可见内容。
首次说明与保存预览应准确表达该区别。
...
- **B-129/AC-01:** 桌面和移动端各能添加图片并发送图片独立消息；理解、文案
  ...
  文件、拖入、相册、粘贴共用导入语义；系统交付 JPEG 可直接使用，没有仅因来源
  产生的未验证警告/补原图动作，实际收到的 HEIC 按 AC-15 拒绝。
...
- **B-129/AC-04:** 外部文件按配置存放；添加后未发送即移除，已成功导入的文件
  仍保留。保留字节与 PA 实际收到的输入一致，不要求与上游相机文件一致，且不被
  预览/发送处理覆盖。已有 vault 图可复用；重命名、移动、删除和同路径内容替换
  均不造成错误关联，PA 保存迁出后多个既有聊天仍解析到同一内容。
- **B-129/AC-05:** 缓存容量受控、清理后可从可用原图重建；删除聊天不删原图
  或正式附件。手动清理提供真实空间与已知引用信息，并限定删除目标。
  迁出的文件不再出现在聊天专用可删除集合中，保留导入来源记录不等于保留删除权限。
- **B-129/AC-06:** Obsidian Sync、Git、iCloud 分别具有可信的处理结果或操作
  ...
  保存预览说明选中图片迁出后遵循普通附件同步设置，未选图片仍在原目录；不修改
  普通附件目录的排除设置，不声称迁出图片仍受 `pa-images` 排除保护。
...
- **B-129/AC-11:** 分别覆盖 Obsidian 根目录、指定目录、笔记同目录、笔记相对
  ...
  选中图片迁出后原路径不再保留持久副本，目标字节与输入一致，聊天和笔记均可用。
  未选图片、预览取消、手动引用、其他插件引用不触发移动；普通库内图片及已经迁出
  的共享图片再次保存时不复制或搬动。覆盖移动成功但登记/链接未完成、并发保存、
  同名不同内容和用户编辑冲突；任一步失败真实返回部分结果，重试认领正确结果，
  不覆盖冲突文件、不丢内容、不重复附件。清缓存或重开会话不破坏迁出后的引用。
...
- **B-129/AC-15:** 下述格式矩阵和移动资源边界具有与支持声明一致的验证证据；
  ...
  直接 HEIC、伪装扩展名/MIME 的 HEIC、库内 HEIC 均不能作为新聊天附件加入，
  不产生新资产或原文件写入，不调用 HEIC 转换器；系统已交付 JPEG 时接收、
  保留并保存该 JPEG。旧 HEIC/保存记录按实施前兼容设计验证，不以删旧数据通过新门禁。
...
| HEIC（含以 HEIF 扩展名交付的 HEIC 内容） | 真实格式识别、所有添加入口拒绝、JPEG 交付路径及旧数据兼容 | 用户于 2026-09-08 取消支持；先转 JPEG 再交给 PA，不登记/写入新 HEIC，不内部转换，不要求保留上游 HEIC |
