...
| 创建日期 | 2026-05-02 |
| 最后回顾 | 2026-05-09，Phase 2D review fixes 已完成：full frontmatter metadata search、hard tool context budget、outline cache-miss fallback，并完成重新部署后的 Obsidian smoke |
...
| 测试状态 | [x] Phase 2D + review fixes 的 targeted/full Jest、type check、lint、build、whitespace、`make deploy` 和新增工具 Obsidian smoke 已通过；Current Note 无 active Markdown file、Thinking scroll 长输出回归、presearch 多样本召回和真实混合工具预算仍作为 follow-up |
...
| UI regression：Thinking 展开与滚动 | Chat UI streaming | [~] Partial | Streaming 过程中 Thinking 可展开；用户滚到上方历史时不被拉回底部；回到底部后自动滚动恢复 | Obsidian 已验证 Thinking 展开与取消路径；长输出 scroll-up / return-to-bottom 完整循环仍待补 |
