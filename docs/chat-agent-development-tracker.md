...
| 创建日期 | 2026-05-02 |
| 最后回顾 | 2026-05-09，Phase 2C Memory presearch、planner prompt 模板转义、fallback relevance guard 和 Obsidian test vault smoke 已完成 |
| 当前阶段 | Phase 2: Read-only Tool Expansion |
...
| 实现状态 | [~] Phase 2A 已实现 tool registry / `search_memory` 工具化；Phase 2B 已实现 `get_current_note_context`；Phase 2C 已实现每轮先做 Memory presearch，再让 planner 基于相关摘录规划 |
| 测试状态 | [x] Phase 2C targeted/full Jest、type check、lint、build、whitespace、`make deploy` 和 Obsidian test vault smoke 已通过；Current Note 工具专项 smoke 与 Thinking/streaming scroll 回归仍作为 Phase 2 follow-up |
...
- [~] 将 tool observations 交给 `PromptBuilder` 做上下文预算和来源约束。当前已支持 Memory 与 current note 两类 typed context，Memory 仍走独立 `memory_content` / `allowed_sources`，current note 不混入 Memory references；统一 `ChatContextItem[]` 抽象 deferred。
...
- [x] 工具失败不阻断普通回答。
- [x] UI 能展示轻量 tool 状态。
- [~] 多工具结果不会突破上下文预算。`search_memory` 继续沿用 Memory 文档数和字符预算，`get_current_note_context` 使用选区/附近内容预算和最多 2 个 current note context；统一跨工具总预算 deferred。
- [x] 模型不能直接构造未注册工具调用。
- [x] 隐式笔记问题可通过 presearch 命中 Memory 并由 planner 直接 answer。

Phase 2 未完成 / deferred：

- [ ] 统一 `ChatContextItem[]` 抽象，把 Memory、current note 和后续工具结果放入同一上下文预算模型。
- [ ] 补 Obsidian UI smoke：`get_current_note_context` 的选区、附近内容、无 active Markdown file、混合 current note + Memory search。
- [ ] 补 Obsidian UI smoke：长 streaming 回答时展开 Thinking、向上滚动、回到底部后的自动跟随恢复。
- [ ] 评估 raw prompt presearch 在长问题、口语化问题中的召回质量，再决定是否引入轻量 query cleanup 或 planner 生成补充 query。
- [ ] 在真实混合工具问题中调优 observation digest 和总上下文预算。
- [ ] Phase 2 后续只读工具候选仍未实现：`search_vault_metadata`、`list_recent_notes`、`read_note_outline`。
...
| 2026-05-09 | Obsidian UI smoke：Current Note 工具 | [ ] Pending | 建议手动验证 Memory ready 时当前笔记问题显示 `Finding related memory` 后可继续 `Reading current note`，选区/附近内容能影响回答；混合问题可继续触发补充 Memory search |
