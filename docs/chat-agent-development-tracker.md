...
| 实现状态 | [x] Phase 2 MVP 已实现 tool registry / `search_memory` / `get_current_note_context` / Memory presearch，并已补齐当前 Memory + current note 的统一 context budget |
...
| Phase 2 | 只读工具扩展 | [~] In progress | MVP 已实现；剩余为 Obsidian smoke、presearch 召回评估、真实混合工具预算调优和后续工具候选 | `docs/chat-agent-phase2-readonly-tools-plan.md` |
...
- [x] 将 tool observations 交给 `PromptBuilder` 做上下文预算和来源约束。当前 Memory 与 current note 都会先转换为 `ChatContextItem[]`，再统一去重、限制总 context 预算；Memory 仍走独立 `memory_content` / `allowed_sources`，current note 不混入 Memory references。
...
- [ ] 后续新增更多工具时，扩展 `tool-note` / 其他 `ChatContextItem` 渲染策略，避免为新工具重新开一条 prompt 注入路径。
...
- [~] 补 Obsidian UI smoke：长 streaming 回答时展开 Thinking、向上滚动、回到底部后的自动跟随恢复。真实 streaming 中 Thinking 展开和取消已验证；完整长输出滚动恢复仍待补。
