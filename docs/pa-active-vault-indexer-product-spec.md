...
The current Memory/VSS system provides local embedding/index infrastructure and
`Memory from your notes`. Active Vault Indexer is the next product layer above
that foundation: it gives Chat, Pagelet, Memory, and Maintenance a shared way to
...
2026-06-28. It should be treated as a product contract for future SDD work, not
as a claim that the runtime already implements these behaviors.
