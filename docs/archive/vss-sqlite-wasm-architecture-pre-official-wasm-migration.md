# VSS SQLite/WASM 架构设计（历史实现稿）

> **Status (2026-07-11)**: Historical — this document preserves the pre-`@sqlite.org/sqlite-wasm` / pre-worker-brute-force design. Ollama, `@sqliteai/sqlite-wasm`, `sqlite-vector`, `vector_init`, and `vector_full_scan` references are not current. Use the [current architecture](../architecture/vss-sqlite-wasm-architecture.md).
## 背景
