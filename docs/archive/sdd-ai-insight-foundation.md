...
| SPEC | 标题 | Phase | 状态 |
| --- | --- | --- | --- |
| SPEC-D1 | Heading-aware chunking | v2.4 | `[x]` Implemented |
| SPEC-D2 | Pagelet VSS integration | v2.4 | `[x]` Implemented |
| SPEC-D3 | Prompt 改进 5 项 | v2.4 | `[x]` Implemented |
| SPEC-D4 | calcSnapshot 增量化 + 检索窗口扩大 | v2.4 | `[x]` Implemented |
| SPEC-D5 | Context Projector + Hygiene | v2.4 | `[x]` Implemented |
| SPEC-D6 | Context Compactor + Budget | v2.5 | `[x]` Implemented |
| SPEC-D7 | Type A 用户画像 (自动提取 + 置信度 + recurrence) | v2.5 | `[x]` Implemented |
| SPEC-D8 | Type C Vault 元认知 (6 维度) + Memory extraction pipeline | v2.5 | `[x]` Implemented |

### Implementation Closeout (2026-06-16)

- D1 uses `src/vss/markdown-chunker.ts` and bumps `VSS_SCHEMA_VERSION` to 2. Existing schema-v1 local indexes are treated as stale and enter the existing user-confirmed Prepare/Update Memory path rather than silently rebuilding.
- D2 injects semantic related notes into Pagelet review input via `src/plugin.ts` and `src/pagelet/pa-review-schemas.ts`; related notes may support `related_notes`, but cannot be used as `source_id`.
- D3 extends `query-rewriter` with `temporal` intent and passes a temporal filter through VSS hybrid search to SQLite FTS/vector result filtering.
- D4 is closed across both source documents: `src/stats/stats-manager.ts` already provides `calcSnapshotIncremental()` with file-count cache fallback and equivalence tests, and the AI Insight analysis #4 retrieval-window constants are expanded in `src/ai-services/pa-agent-runtime.ts`.
- D5/D6 are implemented under `src/ai-services/context/`; current Projector support is origin annotation, deterministic history projection/compaction, injected `<user_profile>` / `<vault_insights>` context, and boundary escaping. Budget records char/token diagnostics and provider usage when provider chunks expose usage; it is not a unified retrieval-constant controller.
- D7/D8 are implemented under `src/ai-services/memory-extraction/`: Type A uses local explicit-preference/correction extraction plus recurrence promotion and IndexedDB storage; Type C uses Obsidian metadata/topology analysis as an internal snapshot and does not write vault notes by default. After the E-series activation work, Vault Insights are summarized into prompt context when `memoryExtractionIncludeVaultInsights` is enabled, which is the current default.
...
graph LR
    ...
    LIMITS --> D5[SPEC-D5<br/>Context Projector + Hygiene]
    D5 --> V24[v2.4 stable]
    D2 --> V24
    D4[SPEC-D4<br/>calcSnapshot + 检索窗口] --> V24

    V24 --> D6[SPEC-D6<br/>Context Compactor + Budget]
    D5 --> D7[SPEC-D7<br/>Type A 用户画像]
    D5 --> D8[SPEC-D8<br/>Type C Vault 元认知]
    D6 --> V25
    D7 --> V25
    D8 --> V25
...
| history 轮次硬上限 | 已移除；`MAX_CHAT_HISTORY_CHARS = 60,000` + compaction summary | 长对话上下文丢失，用户需重复信息 |
...
4. **calcSnapshot 增量化 + 检索窗口扩大** (SPEC-D4) — 减少重复计算开销，并扩大 Memory 检索覆盖
5. **Context 限制放宽** — 常量调整，基于 128K 模型基线
6. **Context Projector** (Phase 1-2) — 投影层，message origin 标注 + history projection + Type A/C 注入准备
...
| `MAX_CHAT_HISTORY_TURNS` | removed | 20 | 不设上限 | 靠 `MAX_CONVERSATIONS=50` 控制总量 |
| `MAX_CHAT_HISTORY_CHARS`（新增） | `pa-agent-runtime.ts` | 无 | 60,000 | history 注入量由 Projector 动态决定 |
...
1. **Context Compactor + Budget** (SPEC-D6) — micro-compaction、history summary、chars/tokens 诊断 + provider usage 回写
2. **Type A 用户画像** (SPEC-D7) — 自动 post-session extraction + 置信度分级 + recurrence 过滤
3. **Type C Vault 元认知** (SPEC-D8) — 6 维度 vault 分析 + Memory extraction pipeline
...
- calcSnapshot 增量化 + 检索窗口扩大 (SPEC-D4)
- Context 限制放宽（常量调整）
...
> 历史占位。当前实现状态见本文件 §0 closeout 与 `v2-post-release-spec-driven-development.md`。
...
### 3.4 calcSnapshot 增量化 + 检索窗口扩大 (SPEC-D4)

> 历史占位。当前实现状态见本文件 §0 closeout 与 `v2-post-release-spec-driven-development.md`。

核心：`AgentControlSnapshot` 计算从全量重建改为增量更新，减少每轮计算开销；Memory 检索窗口按 AI Insight analysis #4 扩大到 8 个文档、4000 字符、12 个 rerank 候选、3 个候选 chunk 和 1000 字符候选摘要。
...
| `PaAgentContextProjector` | 标注 message origins + 投影 chat history + 注入 Type A/C 上下文 | v2.4 |
...
| `PaAgentContextBudget` | chars/tokens 追踪 + provider usage 回写 + near-limit 诊断；`PaAgentContextManager` 消费该诊断触发更激进 micro-compaction | v2.5 |
...
flowchart TD
    ...
    B --> H["forPrompt()"]
    H --> I["Projector: 标注 origins + 注入 context"]
...
- 当前实现保留 runtime 侧的 host context / skill catalog 格式化；Projector 负责 chat history 投影与 Type A/C injected context 包装。

**Host Context Diffing（未实现，后续项）**

- 当前实现每轮使用 runtime 侧生成的 host context，不做字段级 diff。
- 若后续需要降低 prompt churn，应新增稳定 snapshot + diff 机制，并补充 provider/cost smoke 证据。
- 参考方向：Codex `record_context_updates()` 差分注入模式。
...
**摘要生成**：
- 当前实现使用 deterministic history summary，避免在未增加新确认 UI 前引入额外 provider 调用
- LLM-based summary + cache remains a future enhancement that needs explicit data/provider/cost disclosure

**History 处理策略**：
- history 存储不设轮次上限（靠 `MAX_CONVERSATIONS = 50` 控制总量）
- `forPrompt()` 时：**最近 ~10 轮完整注入** + 更早的 deterministic 摘要
- 摘要标注 `compaction_summary` origin

**安全机制**：
- history 超预算时按 whole-message 边界裁剪并重新生成 wrapper，不对已格式化 XML/JSON raw slice
...
**预算超限联动**：
- `PaAgentContextBudget.snapshot()` 返回 `nearObservationLimit` 与 usage ratio。
- `PaAgentContextManager.forPrompt()` 在 `nearObservationLimit` 时使用 `targetRatio: 0.4` 重新执行 micro-compaction 并重新 snapshot。
- 预算信息通过 `forPrompt()` 返回，可用于诊断日志；当前不统一管控 Memory 检索窗口常量。

**Provider usage 回写**：
- 从 model response 的 usage 字段读取实际 token 消耗
- 回写到 Budget tracker，作为后续 snapshot diagnostics 暴露
- 参考方向：Hermes `ContextCompressor` — 用 actual usage 避免对粗估过度反应
...
| 用户可见性 | 内部 profile snapshot；Settings 查看/编辑是后续 UI 工作 |
| 容量管理 | 当前 local-first 上限截断；LLM consolidation 需要后续确认 UI |
...
- 触发条件：
  - vault 变化事件（`create` / `modify` / `delete` / `rename`）— 增量更新受影响维度
  - 每 **24h** 完整刷新全部 6 维度
...
| 属性 | 设计 |
| --- | --- |
| 存储位置 | 插件内部 snapshot；默认不写 vault 笔记 |
| 索引方式 | 不进入 VSS，避免自动写用户 vault 和二次索引 churn |
| 注入方式 | 默认不 always-loaded；后续如接入 prompt 需按需注入并明确 sandbox/data disclosure |
| 用户可见性 | 后续 UI/导出能力另行设计 |
...
flowchart TB
    ...
    subgraph TypeC_Pipeline["Type C: Vault Metacognition"]
        direction TB
        TC1["vault 变化事件<br/>create/modify/delete/rename"]
        TC2["增量更新<br/>受影响维度"]
        TC3["每 24h<br/>定时刷新"]
        TC4["完整 6 维度分析<br/>(含 LLM 分析)"]
        TC5["更新内部<br/>snapshot"]
...
| 存储目标 | IndexedDB | internal snapshot |
| 注入方式 | always-loaded | 当前默认启用时以摘要注入 prompt；viewer 使用内部完整 markdown |
...
| `src/ai-services/context/PaAgentContextManager.ts` | 新增 | `PaAgentContextManager` 组合类 |
| `src/ai-services/context/PaAgentContextProjector.ts` | 新增 | `PaAgentContextProjector` 投影层，负责 history projection + injected context |
| `src/ai-services/context/PaAgentContextHygiene.ts` | 新增 | `PaAgentContextHygiene` 清洁层 |

### 4.2 v2.5 改动范围

| 文件 | 改动类型 | 说明 |
| --- | --- | --- |
| `src/ai-services/context/PaAgentContextCompactor.ts` | 新增 | `PaAgentContextCompactor` 压缩层 |
| `src/ai-services/context/PaAgentContextBudget.ts` | 新增 | `PaAgentContextBudget` 预算追踪 |
| `src/ai-services/memory-extraction/type-a-extractor.ts` | 新增 | Type A 用户画像提取 |
| `src/ai-services/memory-extraction/profile-store.ts` | 新增 | Type A vault-scoped IndexedDB 存储 |
| `src/ai-services/memory-extraction/type-c-analyzer.ts` | 新增 | Type C vault 元认知分析 |
| `src/ai-services/memory-extraction/extraction-scheduler.ts` | 新增 | Memory extraction pipeline 调度 |
| `src/ai-services/context/PaAgentContextManager.ts` | 新增 | 接入 Projector/Hygiene/Compactor/Budget delegates |
| `src/ai-services/context/PaAgentContextProjector.ts` | 新增 | 接入 Type A/C 注入 |
| `src/ai-services/pa-agent-runtime.ts` | 修改 | Context manager 注入、provider usage 诊断、预算常量更新 |
| `src/chat/chat-view.ts` | 修改 | 对话持久化后通知 extraction pipeline |
| `src/plugin.ts` | 修改 | 启动/dispose extraction scheduler；Pagelet VSS related-note 查询 |
...
| Full compaction | 单元测试 | older history 摘要生成；recent history 保留；origin 标注 |
| Budget | 单元测试 | chars/tokens 追踪准确；provider usage 诊断输出与 budget 回写 |
| Type A | 集成测试 | 置信度分级正确；IndexedDB store seam 可持久化 |
| Type C | 集成测试 | 6 维度数据提取；默认不写 vault / 不注入 prompt |
| 端到端 | Obsidian smoke | 跨对话记忆生效；Type C scheduler 不扰动用户 vault |
...
| Type A 提取引入额外 LLM 成本 | 低 | 当前实现为 local-first explicit/correction 提取，不额外调用 provider；频率可配置 |
| Type C LLM 分析成本 | 低 | 当前实现为 Obsidian metadata/topology 分析，不额外调用 provider；24h 低频刷新 |
...
3. D4（calcSnapshot 增量化 + 检索窗口扩大）sequential on master
4. Context 限制放宽（常量调整）sequential on master
5. Projector Phase 1（`forPrompt()` 提取 + 行为等价测试）sequential on master
6. Projector Phase 2（origins 标注 + injected context 包装；host context diffing 后续评估）sequential on master
...
| Type C 存储位置 | 已调整：插件内部 snapshot | 默认不写用户 vault；避免绕过 Memory confirmation copy |
...
| Host context diffing 粒度 | 按字段 diff / 全量 hash 比较 / 继续全量注入 | Prompt churn 与实现复杂度 | 后续有明确成本证据时再评估 |
