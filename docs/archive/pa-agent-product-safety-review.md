...
- `docs/development/proposals/operations-agent/operations-agent-plan.md` keeps write, command, script, and local MCP work out of PA Agent v1.
...
- Telemetry baseline real aggregate data is not collected yet. The v1 events are local/default-off and content-free; `docs/operations/pa-agent-telemetry-baseline.md` defines the post-ship collection boundary before Operations Agent planning uses usage data.
