# PA Agent Architecture Plan（历史实施稿）

## Status And Source Of Truth

> **Status (2026-07-11): Historical.** This is the pre-v2 implementation plan and preserves migration rationale. `ChatAgentRuntime`, feature-flag migration, and rollback-path language are not current. Use the [current PA Agent architecture](../architecture/pa-agent-architecture-plan.md).
...
| Operations Agent 时间表 | v1 ship 后立刻启动 | v1 期间不做 write action 设计，但 ship 同周开始 `docs/development/proposals/operations-agent/operations-agent-plan.md`。 |
...
- 三层渐进加载：L1 frontmatter 始终进 system prompt；L2 `SKILL.md` body 由模型 `load_skill` tool call 触发加载（详见 [PA Agent Design Completion Audit §4.5](../archive/pa-agent-design-completion-audit.md)）；L3 `references/*.md` 在 body 显式引用时按需加载。早期方案 §445 写的 "L2 在 SkillRouter 选中后加载" 已被 A3 决策取代——bag-of-words router 召回率不足以承担 L2 选择，模型自判通过 description 是更好的 router。
