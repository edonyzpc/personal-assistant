...
observerRemoved、原会话指针/笔记恢复、errors=[]。
整合时复核发现观察脚本使用了错误的 leaf type `llm-chat`，实际为 `sidellm-view`：
回执中的 temporaryChatsRemaining=0 不能证明 Chat leaf 已关闭，撤回该项解释。
原始回执保留不改；该观察器缺陷不影响实际导入、字节、取消、已移除监听器及设置恢复证据，
也不据此重跑手机输入矩阵。
...
2026-09-09（Asia/Singapore），开发分支 closeout 提交
`6b13c6843b627db2e119220dff35aaf49f6e66f9` 已推送；本地 master
`e2c746f5456ea47ca121145ff5d63b1b4fc4359c` 采用保留双亲的 merge。
Settings 冲突保留主线 Pagelet 容器与新版分组断言，加入 Chat images 管理入口；
styles.css 从合并后的源样式重新生成。未 rebase 或改写已推送提交。

- PASS：Settings / ChatView / menu 定向 3 suites / 477 tests；完整 make deploy
  覆盖 guards、lint、production build/typecheck、243 suites / 6459 tests，
  Jest 105.889s、自然退出 0；DOM 源码扫描无匹配。
- PASS：Mac test 实际加载 main.js SHA-256
  `3ac6e826e69d68c7020026d1412720bc1e03a53ada53c39fe20e12396a563740`，
  identity blocker=null。Settings 真实点击 Manage saved originals 打开管理窗口；
  Show Pet 关闭/开启时 Pet corner 消失/重现，管理入口再次打开成功，原设置 true 已恢复。
  此路径验证的是局部条件显示，不声称调用了 refreshPageletSettingsIfVisible。
- PASS：CLI mobile 390×844 下真实点击 Chat 菜单及 Add images，菜单和 From Files /
  From vault 文案完整；取消后保留会话。重开独立 Reused.md，迁出 JPEG 可见，
  img complete=true、320×180。未重复导入、provider 请求或更改真实数据。
- iPhone 原生输入证据仍限定于上文 frozen build；本次合并未重新部署 iCloud 或声称
  手机加载此新 SHA。合并交叉风险在 Mac 设置与共享 UI 完成验证，未重复手机矩阵。
  Legacy HEIC / pending save 应用夹具仍为 NOT TESTED，保留 T-006。
- 清理 PASS：dev:errors 无捕获错误；debug/mobile 关闭，原笔记、原会话与 Show Pet
  设置恢复。清除设备模拟后需原生窗口缩放一次才恢复完整桌面内容区域，已目视确认。
  最终 docs:check PASS（200 Markdown / 1692 links，四条既有 advisory），diff 检查通过。
