...
| 属性 | 值 |
|------|-----|
| 状态 | `[R]` Ready（activation plan §3.1 + §3.2 已充分描述） |
...
| 属性 | 值 |
|------|-----|
| 状态 | `[D]` Drafting（AD-2 要求独立 SDD） |
| SDD | **待起草**：`docs/sdd-graph-aware-retrieval.md` |
...
| 属性 | 值 |
|------|-----|
| 状态 | `[R]` Ready（activation plan §4.2 已充分描述） |
...
| 属性 | 值 |
|------|-----|
| 状态 | `[R]` Ready（activation plan §4.3 + §4.4 已充分描述） |
| SDD | 无需独立 SDD |
...
| 属性 | 值 |
|------|-----|
| 状态 | `[D]` Drafting（需要 SDD 细化 LLM prompt + cost + fallback） |
| SDD | **待起草**：`docs/sdd-type-a-llm-extraction.md` |
...
| 属性 | 值 |
|------|-----|
| 状态 | `[R]` Ready（activation plan §5.2/5.3/5.4 已充分描述） |
| SDD | 无需独立 SDD |
...
```
[D] Draft → [R] Ready → review → [A] Approved → [~] Implementing → test → code review → make deploy → [S] Smoke → [x] Done
```

| SPEC | 当前状态 | 下一步 |
|------|---------|--------|
| E1 | `[R]` Ready | Review → `[A]` → 实现 |
| E2 | `[D]` Drafting | 编写 SDD → `[R]` → Review |
| E3 | `[R]` Ready | 等 E1 merge → Review → `[A]` → 实现 |
| E4 | `[R]` Ready | Review → `[A]` → 实现（可与 E2/E3 并行） |
| E5 | `[D]` Drafting | 编写 SDD → `[R]` → Review |
| E6 | `[R]` Ready | 等 E1 merge → Review → `[A]` → 实现 |

---

## 6. SPEC Index 更新（追加到 v2-post-release-spec-driven-development.md）

以下条目在 E-series SPEC 通过 review 后追加到 SPEC Index：

```markdown
| SPEC-E1 | Vault Insights 激活 + Discovery 专用流程 | `[ ]` | v2.5 | SPEC-D8 | [`ai-insight-activation-plan.md`](./ai-insight-activation-plan.md) §3 | `settings.ts`, `plugin.ts`, `pagelet/orchestrator.ts`, `pagelet/panel/` | vault insights 注入 prompt + Discovery 返回 connections/gaps/insights |
| SPEC-E2 | 图感知检索 (1-hop link expansion) | `[D]` | v2.5 | SPEC-E1 | [`sdd-graph-aware-retrieval.md`](./sdd-graph-aware-retrieval.md) | `pa-agent-runtime.ts`, `plugin.ts` | 搜索命中笔记的 wikilink 目标出现在结果中 |
| SPEC-E3 | Pagelet VSS 全场景覆盖 | `[ ]` | v2.5 | SPEC-E1 | [`ai-insight-activation-plan.md`](./ai-insight-activation-plan.md) §4.2 | `plugin.ts`, `pagelet/PeriodicSummaryFlow.ts`, `pagelet/output/` | Preload + Periodic Summary 包含跨笔记 VSS 上下文 |
| SPEC-E4 | Budget→Compaction 联动 + 测试补齐 | `[ ]` | v2.5 | None | [`ai-insight-activation-plan.md`](./ai-insight-activation-plan.md) §4.3-4.4 | `context/*`, `__tests__/*` | Budget 超限触发二次压缩 + 4 类新测试通过 |
| SPEC-E5 | Type A LLM 后台提取 | `[D]` | v2.5 | SPEC-E1 | [`sdd-type-a-llm-extraction.md`](./sdd-type-a-llm-extraction.md) | `memory-extraction/*`, `plugin.ts` | inferred_behavior 候选出现 + LLM fallback + cost disclosure |
| SPEC-E6 | 语义聚类 + temporal range + 死代码清理 | `[ ]` | v2.5 | SPEC-E1 | [`ai-insight-activation-plan.md`](./ai-insight-activation-plan.md) §5.2-5.4 | `vss/*`, `memory-extraction/type-c-analyzer.ts`, `query-rewriter.ts` | Topic Clusters 语义化 + range: temporal + 死代码删除 |
```
...
|----------|------|---------|---------|------|
| `docs/sdd-graph-aware-retrieval.md` | E2 | 展开方向/深度/膨胀控制/插入点/性能预算（8 个设计问题，含 [P3] 非 LLM 延迟 ≤ 2s） | E1 实现期间 | 阻塞 E2 实现 |
| `docs/sdd-type-a-llm-extraction.md` | E5 | Prompt 设计/截断/model/cost disclosure/fallback/schema 兼容/移动端 guard/cost tracking（9 个设计问题，含 [U3][P4][U4]） | Phase 2 期间 | 阻塞 E5 实现 |
...
| P1 | 性能 | Projector diffing 有效性 | 可接受 | 第 2+ 轮 ~47 chars 替代 ~3000 chars | 无需修改 |
