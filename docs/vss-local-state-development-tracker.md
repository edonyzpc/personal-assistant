...
| 2026-05-30 | Obsidian smoke | Passed with scoped deferrals | `make deploy` passed, Obsidian test-vault Memory diagnostics recovered to `Ready`, `Update memory now` succeeded with notes unchanged, and diagnostics reported `22 chunks across 21 files`, `sqlite-wasm-opfs-sahpool`, persistent storage, and `Maintenance Up to date`. Reset/delete-old-cache confirmations were opened and cancelled. |
| 2026-05-30 | High-risk manual checks | Deferred | `Update plugins`, `Update themes`, AI Featured Images, and actual Memory reset/delete-old-cache execution were not run because they modify installed assets, call network/API, or delete local cache. |
