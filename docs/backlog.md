...
| B-116 | Pagelet 原 B-108 dogfood 延后范围 | 只在真实 dogfood 证据命中时分别重启：double-Ctrl 需跨平台冲突与实体操作证据；Chat Quick Command 需证明现有入口摩擦；`pa-related` frontmatter Sync 需先解决多设备冲突；Weekly Review compatibility helper 需兼容性证明后才移除。Pattern LLM 已由 B-119 接管，不再是本条重启条件。`replace_selection` 继续由 B-101 / T-003 的写操作边界治理，不在此重复授权 | [Historical tracker](./archive/pa-product-redesign-development-tracker.md), [B-119 Product Spec](./product/specs/pa-insight-enhancement-layer-product-spec.md) |
...
| B-123 | Pagelet `Deep Discover` 只读盲测 | 先完成 B-121 closeout 与 B-119 有界能力 dogfood；只有现有 pipeline 仍无法发现高价值跨笔记盲区时，才用冻结 snapshot、vault-local source、零 web/零 write 的小实验与现状盲比，不继承“全量 Agent 化、统一 UI、30 calls/180s”的草稿假设 | [PA Agent architecture](./architecture/pa-agent-architecture-plan.md), [B-119 Product Spec](./product/specs/pa-insight-enhancement-layer-product-spec.md) |
