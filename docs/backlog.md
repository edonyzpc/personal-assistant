...
| B-121 | Quiet Bubble 普通空状态的 3 秒价值 | B-118 保持现状；只有真实使用证明 “Intentionally Quiet” 空状态仍让用户困惑或无法理解下一步时，才重新决定是否增加解释、入口或内容层级，不与 Recap/Recall ready-state 混改 | [B-118 Product Spec](./product/specs/pagelet-ui-ux-hardening-product-spec.md), [B-118 Tracker SG-07c](./development/active/pagelet-ui-ux-optimization/tracker.md#resolved-decisions) |
