...
>
> **H-B3.2 settings-layer 闭环（更新 2026-06-04）**：`PaReviewToolProvider` 的 `targetConfinement.allowedRoots` 不直接信任 framework Gate 1，而由 settings 层的 `normalizeReviewsFolder` 在 data.json 写入边界处先行 fail-closed 把 10 类越界形状（`empty / too_long / absolute_path / drive_letter / parent_traversal / obsidian_config / forbidden_dotfolder / control_chars / invisible_chars / trailing_dot_or_space`）落到默认 `.pagelet`。实现位置：`src/settings/pagelet/index.ts:243` `normalizeReviewsFolder` + `src/settings/pagelet/index.ts:192` `mergePageletSettings` boot-time merge + `src/plugin.ts:651` `loadSettings` + `src/plugin.ts:111` 的一次性迁移 Notice（localStorage flag `pa-pagelet-reviews-folder-migration-v1`）。framework SDD §8.3 描述的攻击面在这一层即被截断；framework Gate 1 仍作为 defense-in-depth 第二道，但缺少内建 `.obsidian`/`.git`/`.trash` 黑名单（依赖 settings 层先行清洗），该 baseline 单独跟踪为 framework follow-up issue（locked `src/ai-services/write-action-framework/**` 编辑窗口打开后处理）。回归测试见 `__tests__/pagelet-settings.test.ts` 47 个 case（覆盖 B1 case-fold / B2 NTFS trailing dot/space / B3 backslash / H2 Cf-category + DEL / S2 forbidden_dotfolder + too_long + typed-Record 渲染 + 全 10 类 EN+ZH i18n 契约穷举测试）。
>
