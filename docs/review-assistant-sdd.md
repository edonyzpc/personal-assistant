...
>
> **H-B3.2 settings-layer 闭环（同 2026-06-03）**：`PaReviewToolProvider` 的 `targetConfinement.allowedRoots` 不直接信任 framework Gate 1，而由 settings 层的 `normalizeReviewsFolder` 在 data.json 写入边界处先行 fail-closed 把 8 类越界形状（`empty / absolute_path / drive_letter / parent_traversal / obsidian_config / control_chars / invisible_chars / trailing_dot_or_space`）落到默认 `.pagelet`。实现位置：`src/settings/pagelet/index.ts:219` `normalizeReviewsFolder` + `src/settings/pagelet/index.ts:170` `mergePageletSettings` boot-time merge + `src/plugin.ts:loadSettings` 的一次性迁移 Notice（localStorage flag `pa-pagelet-reviews-folder-migration-v1`）。framework SDD §8.3 描述的攻击面在这一层即被截断；framework Gate 1 仅作为 defense-in-depth 第二道。回归测试见 `__tests__/pagelet-settings.test.ts` 对应 describe 块（包含 B1 case-fold / B2 NTFS trailing dot/space / B3 backslash / H2 Cf-category 5 类绕过覆盖）。
>
