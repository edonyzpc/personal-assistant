...
8. `./src/vss/sqlite-inline-assets.ts`:
   ...
   - WASM 懒加载逻辑(见 `sdd-wasm-lazy-load.md`)保持不变
...
- `./src/vss/sqlite-inline-assets.ts` —— WASM 路径调整(配合 `sdd-wasm-lazy-load.md` 保持懒加载)
...
**阅读参考:**
- `./docs/sdd-wasm-lazy-load.md`(WASM 懒加载,本迁移需保持兼容)
- `./docs/sdd-chat-tools-split.md`(memory 工具上游消费者)
