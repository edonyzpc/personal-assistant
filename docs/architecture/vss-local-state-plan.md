...
| Ready/policy atomicity | A prepared index is not usable ready if policy/lifecycle admission fails; rollback restores the guarded recovery reason before the run can be exposed as ready |
...
- Abort and total failure retain the guard. Successful rebuild plus successful Memory policy/lifecycle admission clears it. If policy persistence or lifecycle admission fails after index preparation, `rollbackPreparedRebuild(reason)` restores non-ready state with the original guarded reason; no usable ready marker may remain.
...
- If saving `memoryApprovalPolicy` or accepting the prepared lifecycle fails after index build, durable compensation removes usable-ready admission and restores the original rebuild guard/reason.
