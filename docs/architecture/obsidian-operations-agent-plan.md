...
| --- | --- | --- |
| `docs/architecture/obsidian-operations-agent-plan.md` | Product and architecture contract for Obsidian operations knowledge, v1A read-only tools, v1B CLI adapter, source boundaries, and safety model. | This wins for this feature family. |
...
| `docs/archive/write-action-design-handoff.md` | Future write-action boundary for preview, confirmation, and audit. | This wins for future writes and command execution. This plan must not weaken it. |
| `docs/architecture/pa-agent-architecture-plan.md` + `docs/architecture/pa-agent-runtime-lifecycle-plan.md` | Current PA Agent runtime + native tool loop reference (replaced the historical Ralpha plan in v2.0.0). | These win for existing runtime behavior until an approved Obsidian Operations SPEC changes it. |
...
Future write or command capabilities must use a separate action family with preview, confirmation, cancellation, and redacted local audit as defined in `docs/archive/write-action-design-handoff.md`.
...
- `PaAgentRuntime` owns host composition; `PaAgentLoop` and the tool dispatcher own canonical turn/tool execution, streaming lifecycle, abort, and terminal metadata.
