...
| Current stage | SPEC-A4 dev cycle green on master HEAD `8352261` 2026-06-01: deprecated flags `paAgentAnswerStreamEnabled` + `nativeToolPlanningSmokeEnabled` removed across `src/settings.ts` / `src/ai-services/chat-service.ts` / `src/plugin.ts` / `__tests__/chat-service.test.ts` + CHANGELOG `## Unreleased / Removed (Breaking)` entry; tsc=0 and jest 4492/4492 still pass. Companion build artifact commit `b8030b9` regenerates `styles.css` for A1's `.pa-chat-config-banner`. master is 33 commits ahead of `origin/master` (was 31 + 2 new). Awaiting user `make deploy` + real Obsidian smoke before flipping A4 to `[x] Done`. v2.2 Step 2 remaining after A4 smoke: SPEC-A2 (command palette) — still `[D]`, sequential on shared `plugin.ts`. SPEC-A3 H-1 sub-item still `[D]` deferred to >= 2026-06-12; SPEC-A6/A7 `[D]` deferred to v2.3/v2.5; SPEC-A8/A9 `[T]` triggered-only. Manual-CN.md mirror + `docs/onboarding-*.png` screenshots remain queued as A1 follow-up docs (non-blocking). |
| Runtime code changes in this pass | SPEC-A4 commit `8352261`: 5 source files + CHANGELOG, net −22 LOC (7 insertions / 29 deletions). No behavioral change — both flags were already no-ops since v2.0.0 (per A4 SDD). Build commit `b8030b9` rebuilds `styles.css` for A1's banner CSS. |
...
| Next required action | (1) User runs `make deploy` from main repo + real Obsidian smoke for SPEC-A4 (verify chat / native tool planning / record-note / search_memory paths still functional;stored-settings with stale `paAgentAnswerStreamEnabled` / `nativeToolPlanningSmokeEnabled` keys load without errors). (2) On smoke pass, flip A4 row to `[x] Done` in tracker. (3) Then start SPEC-A2 (command palette cleanup: Memory advanced commands behind `showAdvancedMemoryControls` toggle; Featured Images limited to qwen). (4) After A2 lands + deploy + smoke, v2.2 batch is complete and `git push origin master` + PR is unblocked. (5) A1 follow-up: capture onboarding screenshots + add Manual-CN.md mirror (non-blocking, can ride alongside A2 PR). |
...
| SPEC-A4 | 无消费者 flag 删除 | `[~]` Implementing @`8352261` (dev+test green;awaiting deploy + smoke) | v2.2 | SPEC-A0 | [`sdd-deprecated-flags-removal.md`](./sdd-deprecated-flags-removal.md) | `./src/settings.ts`、`./src/ai-services/chat-service.ts`、`./src/plugin.ts` migrateSettings、`./__tests__/chat-service.test.ts`、`./CHANGELOG.md` Unreleased | ✅ `paAgentAnswerStreamEnabled` / `nativeToolPlanningSmokeEnabled` 字段及全部消费点已删除;CHANGELOG breaking 已记录。Smoke 待用户 `make deploy` 后真机验证。 |
