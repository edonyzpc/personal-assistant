...
| Current stage | v2.2 Step 2 in progress: SPEC-A1 (chat onboarding, P0 #1+#2+#4) dev cycle complete on master 2026-06-01 — Commit 1 ribbon redirect (`6ed1ea8`), Commit 2 empty-state config banner (`db0db7b`), Commit 3 README + Manual chat docs (`b251962`). tsc + jest 4492/4492 green on master HEAD `b251962`. SPEC-A1 sits at `[~] Implementing`, awaiting `make deploy` + Obsidian smoke before flipping to `[x] Done`. Step 1 (SPEC-A3 PR-1 + SPEC-A5) still awaiting same combined smoke window — now grouped with A1 to avoid two separate Obsidian smoke cycles. master is 31 commits ahead of `origin/master`; rollback tag `pre-v2.1.2-merge` retained at `6a262d8`. SPEC-A0 closed; SPEC-A2/A4 remain `[D]` awaiting A1 smoke pass + plugin.ts sequential ordering; SPEC-A3 H-1 sub-item still `[D]` deferred to >= 2026-06-12; SPEC-A6/A7 `[D]` deferred to v2.3/v2.5; SPEC-A8/A9 `[T]` triggered-only. |
| Runtime code changes in this pass | Yes (v2.2 Step 2 — A1). SPEC-A1 Commit 1 (`6ed1ea8`): `src/plugin.ts:160-170` ribbon icon left-click now calls `this.activeChatView()` (existing helper at `:693`); right-click `contextmenu` listener preserves `PluginControlModal` access; tooltip updated to "Open AI Chat (right-click for plugin controls)". `__tests__/plugin-record-note.test.ts:245` mock extended with `addEventListener: jest.fn()`. SPEC-A1 Commit 2 (`db0db7b`): `src/chat/chat-view.ts:829-883` `renderEmptyState` closure now consults `this.plugin.getAISetupIssue()` (existing helper at `plugin.ts:1222`) and renders banner with welcome title, specific issue text, and "Open Settings" button reusing the existing `app.setting.openTabById('personal-assistant')` pattern from `chat-view.ts:805-819`. `src/custom.pcss:1907+` adds `.pa-chat-config-banner` styling tweak (gap + max-width). SPEC-A1 Commit 3 (`b251962`): `README.md` adds "AI Chat in 60 seconds" section between existing chat NOTE and gallery; `Manual.md` gains complete "AI Chat" chapter (Description / Entry / Configuration / Sending / Citations / Web search / Memory / FAQ). Manual-CN.md and onboarding screenshots deferred to follow-up. Step 1 (A3 PR-1 + A5) state unchanged from prior pass. |
| Open contract decisions | None. All 5 original decisions + Q1-Q8 拍板 are frozen in the decision record. |
| Blocked implementation areas | SPEC-A2/A4 (v2.2 Step 2 — gated on A1 smoke pass + sequential plugin.ts ordering); SPEC-A3 H-1 (>= 2026-06-12); SPEC-A6 (v2.3 + spike); SPEC-A7 (v2.5); SPEC-A8/A9 (triggered). `git push origin master` deferred per user. |
| Next required action | (1) Run `make deploy` to push master HEAD `b251962` to test vault. (2) Execute combined Obsidian smoke for A3 PR-1 + A5 + A1: ribbon left-click → chat view; right-click → modal; chat empty state without token shows banner + Open Settings button; banner disappears after configuration; capability dispatch / search_memory / WebSearch / record-note unchanged; jest config behavior (Step 1). (3) On smoke pass: flip SPEC-A3(non-H-1), SPEC-A5, and SPEC-A1 to `[x] Done`; capture onboarding screenshots; start SPEC-A4 (flag removal) → SPEC-A2 (command palette) sequentially. On smoke fail: open SmokeFix per loop rules; rollback anchor `pre-v2.1.2-merge` → `6a262d8` still valid. |
...
| SPEC-A1 | Chat onboarding 链路(P0 #1+#2+#4) | `[~]` Commits 1-3 merged @`6ed1ea8`/`db0db7b`/`b251962`(awaiting smoke) | v2.2 批 1 | SPEC-A0 | [`sdd-chat-onboarding-flow.md`](./sdd-chat-onboarding-flow.md) | `./src/plugin.ts` ribbon(left=chat,right=modal)、`./src/chat/chat-view.ts` empty state(setupIssue banner)、`./src/custom.pcss` `.pa-chat-config-banner`、`README.md` "AI Chat in 60 seconds"、`Manual.md` AI Chat chapter | Ribbon 直达 chat;空状态 banner 引导配置;README + Manual Chat 章节齐备。 |
...
| SPEC-A3(non-H-1) | Self-review 2026-06-01 (1-line scope, dry-run validated) | ✓ 2026-06-01 commit `0ec8b92` (worktree-agent-a076c1ec001c1c741) | ✓ tsc=0 + jest 916 (throwaway) + jest 4492 (master) on 2026-06-01 | Skipped — 1-line scope; residual risk: jest coverage threshold (master commit `046774b`) is inert when collectCoverage off (acceptable per A3 SDD) | Pending — `make deploy` to test vault | Pending(record-note callout 不退化 + jest coverage 默认关) | Merged into master at `cee367a` 2026-06-01;awaiting deploy + smoke。P0 #5/#6 closed by decision (no code change)。 |
...
| SPEC-A5 | Subagent review 2026-06-01: Approve (deviations: factories unmodified, parity layer added at registry edge — defensible per `__tests__/obsidian-operations-tools.test.ts:208` direct registry.execute() callsite;LOC delta accepted) | ✓ 2026-06-01 commits `9129ae6` → `dfa3f91` → `285d8f1` → `d98ceb2` (worktree-agent-a16cf09d50baee0e9) | ✓ tsc=0 + jest 916 (throwaway) + jest 4492 (master) on 2026-06-01 | ✓ Subagent Approve, no blockers (LOC delta −7 net source vs −500 SDD prediction;user accepted) | Pending — `make deploy` to test vault | Pending(全 capability 调用链行为不变 + chat / search_memory / WebSearch / record-note) | Merged into master at `6cd1042` 2026-06-01;awaiting deploy + smoke。 |
