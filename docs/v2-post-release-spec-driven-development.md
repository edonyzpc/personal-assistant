...
| Current stage | v2.2 Step 1 + Step 2-A1 closed: combined Obsidian smoke passed on master HEAD `13dbb2b` 2026-06-01 — SPEC-A3 (PR-1, P0 #7 jest collectCoverage), SPEC-A5 (ToolRegistry/CoreToolProvider collapse, decision ②), and SPEC-A1 (chat onboarding P0 #1+#2+#4) all flipped to `[x] Done`. master is 31 commits ahead of `origin/master`; rollback tag `pre-v2.1.2-merge` retained at `6a262d8`. SPEC-A0 closed. v2.2 Step 2 remaining: SPEC-A4 (flag removal) and SPEC-A2 (command palette) — both `[D]`, sequential on shared `plugin.ts`. SPEC-A3 H-1 sub-item still `[D]` deferred to >= 2026-06-12; SPEC-A6/A7 `[D]` deferred to v2.3/v2.5; SPEC-A8/A9 `[T]` triggered-only. Manual-CN.md mirror + `docs/onboarding-*.png` screenshots remain queued as A1 follow-up docs (non-blocking). |
| Runtime code changes in this pass | Closed: A3 PR-1, A5, A1 are all `[x] Done` after smoke. No new runtime code in this update — tracker closeout only. Prior changeset summary preserved in Review Log + Verification Log. |
...
| Next required action | (1) Start SPEC-A4 (deprecated flag removal: `paAgentAnswerStreamEnabled` + `nativeToolPlanningSmokeEnabled`) — touches `src/settings.ts` + `src/ai-services/chat-service.ts` + `src/plugin.ts` migrateSettings; small, isolated, no UI surface. (2) Then SPEC-A2 (command palette cleanup: Memory advanced commands behind `showAdvancedMemoryControls` toggle; Featured Images limited to qwen). (3) After A4 + A2 land + their own deploy + smoke, v2.2 batch is complete and `git push origin master` + PR is unblocked. (4) A1 follow-up: capture onboarding screenshots + add Manual-CN.md mirror (non-blocking, can ride alongside A4 or A2 PR). |
