...
| Current stage | v2.2 Step 1 dev cycle closed: SPEC-A3 (PR-1 portion, P0 #7 jest collectCoverage off) and SPEC-A5 (ToolRegistry/CoreToolProvider collapse, decision ②) merged into master on 2026-06-01 (commits `cee367a`, `6cd1042`; docs umbrella `510efcf`). master is 28 commits ahead of `origin/master`; rollback tag `pre-v2.1.2-merge` retained at `6a262d8`. tsc + jest 4492/4492 green on master HEAD `510efcf`. Both SPECs sit at `[~] Implementing`, awaiting `make deploy` + Obsidian smoke before flipping to `[x] Done`. SPEC-A0 closed; SPEC-A1/A2/A4 remain `[D]` awaiting v2.2 Step 2 (sequential, plugin.ts shared); SPEC-A3 H-1 sub-item still `[D]` deferred to >= 2026-06-12; SPEC-A6/A7 `[D]` deferred to v2.3/v2.5; SPEC-A8/A9 `[T]` triggered-only. |
| Runtime code changes in this pass | Yes (v2.2 Step 1). SPEC-A3 PR-1 portion: `jest.config.js` `collectCoverage` commented out so coverage is opt-in via `--coverage`. Coexists with master commit `046774b` coverage threshold floor (75/71/74/75) which now applies only when coverage is explicitly requested. SPEC-A5: deletes `src/ai-services/core-tool-provider.ts`, slims `src/ai-services/chat-tool-registry.ts` (removes `ToolRegistry` class), expands `src/ai-services/capability-adapter.ts` parity layer, adds `AgentCapabilityKind` doc comment in `src/ai-services/capability-types.ts`, and switches `src/ai-services/pa-agent-runtime.ts` to inject capabilities directly into `CapabilityRegistry`. `policy-engine.ts:35` action defense untouched. Net source LOC −7 (vs SDD prediction −500 — deviation accepted; see Review Log). |
| Open contract decisions | None. All 5 original decisions + Q1-Q8 拍板 are frozen in the decision record. |
| Blocked implementation areas | SPEC-A1/A2/A4 (v2.2 Step 2 — gated on Step 1 smoke pass + sequential plugin.ts ordering); SPEC-A3 H-1 (>= 2026-06-12); SPEC-A6 (v2.3 + spike); SPEC-A7 (v2.5); SPEC-A8/A9 (triggered). `git push origin master` deferred per user. |
| Next required action | (1) Run `make deploy` to push master HEAD `510efcf` to test vault. (2) Execute Obsidian smoke checklist for the merged SPEC-A3 + SPEC-A5 tree (chat / capability dispatch / search_memory / WebSearch unchanged; jest config behavior). (3) On smoke pass: flip SPEC-A3(non-H-1) and SPEC-A5 to `[x] Done` and start v2.2 Step 2 = SPEC-A1 → SPEC-A4 → SPEC-A2 sequential (plugin.ts shared file). On smoke fail: open SmokeFix per loop rules and revert via `pre-v2.1.2-merge` tag if needed. |
