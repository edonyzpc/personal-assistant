...
| Next required action | (1) SPEC-B2: release/stable graduation remains pending; 2026-06-18 post-commit redeploy + CLI runtime check passed, but Computer Use click-through smoke still timed out, macOS Accessibility exposed `0` Obsidian windows, and iOS real-device verification is still not refreshed in this pass。(2) v2.2.0-beta.2 BRAT 发布 + 灰度。(3) v2.2.0 stable 发布。(4) A1 follow-up: Manual-CN.md mirror + onboarding screenshots (non-blocking)。 |
...
| SPEC-D5 | Context Projector(Phase 1 提取 + Phase 2 Hygiene) | `[x]` Implemented | v2.4 | None | [`sdd-ai-insight-foundation.md`](./sdd-ai-insight-foundation.md) | `src/ai-services/context/*`、`src/ai-services/pa-agent-runtime.ts` | `forPrompt()` 边界 + 诊断 metrics；status-only/orphan 过滤；Type A/C 注入入口。 |
...
| SPEC-B2 | N/A (gate, not implementation) | N/A | N/A | N/A | Pending(beta.2 + soak + stable) | Pending(全量 smoke + OQ002 provider) | Unblocked — B1 + A3(H-1) + A2 all done。等 `make deploy` + smoke。 |
