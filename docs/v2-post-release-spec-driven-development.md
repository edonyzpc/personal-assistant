...
| Next required action | (1) v2.7 合并 release: Provider OQ002 矩阵验证 + iOS 真机验证 + Pagelet smoke checklist + manifest 版本号对齐 + CHANGELOG/Release Notes 编写。(2) A1 follow-up: Manual-CN.md mirror + onboarding screenshots (non-blocking)。 |
...
| SPEC | SPEC Review | Dev | Test | Code Review | Deploy | Smoke | Fix / Disposition |
| --- | --- | --- | --- | --- | --- | --- | --- |
| SPEC-C1 | Self([`operations-agent-mode-sdd.md`](./docs/operations-agent-mode-sdd.md)) | ✓ C1-P1~P5 `7befc18`→`f2267d6` + review fixes | ✓ 1893 tests(含 WAF 5 spec files) | Self + review fixes `bf7e56e`/`8f4d675`/`917edbf` | ✓ `make deploy` | Pending(v2.7 合并 smoke) | Done; Framework v1 dogfooding pass 解除 8 周时间锁。 |
