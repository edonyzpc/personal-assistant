...
| 能力 | 代码状态 | 产品状态 |
|------|---------|---------|
| Type C vault insights → prompt | 注入管道完整（Projector + escapeTaggedBoundary + summarize） | `includeVaultInsightsInPrompt` 默认 false，未传入 |
| Discovery 专用 prompt | `buildDiscoveryPrompt()` 完整，含 connections/gaps/insight 结构化输出 | 死代码，Discovery 走通用 review 流程 |
| Backlink 查找 | `findBacklinksForPath()` 已实现 | 仅在 `inspectNote` tool 中使用，搜索管线不用 |
| Discovery 面板渲染 | `DiscoveryResult` / `NoteConnection` 类型 + panel 渲染函数已存在 | 未接入专用数据源 |
| 对话元数据 | `contextUsed` / `activityDetails` / `providerReasoningObserved` 已持久化 | Type A 提取完全忽略，只看用户文本 |
...
**当前状态**：`type-c-analyzer.ts` 分析 7 维度 → `extraction-scheduler.ts` 存储 markdown → `getPromptContext()` 准备注入 → `PaAgentContextProjector.ts` 格式化 `<vault_insights>` 标签 — 全链路代码完整，但 `includeVaultInsightsInPrompt` 默认 false 且未传入。
...
**当前状态**：`buildDiscoveryPrompt()` 完整但是死代码。Discovery 走通用 review 流程（`analyzeCurrentNote()` → `createForegroundAnalyzeCallback`），得到通用建议而非"发现隐藏联系"。
...
| 用户确认 UI | **[Review U3 must-fix]** Settings 中的 `memoryExtractionEnabled` 已作为总开关。当前 Notice 说 "Data is stored locally"，**必须更新为明确告知有后台 API 调用**。建议文案："Uses your configured AI model to analyze conversations in the background, which incurs API costs. Preferences are stored locally." |
...
| `src/locales/plugin/` | **[must-fix]** 更新 `plugin.memoryExtraction.enabledNotice` 和 `plugin.memoryExtraction.settings.enabled.desc`，明确 API 调用 + cost |
