...
| [Codex task prompt](./codex-task-prompt.md) | 按任务范围与当前契约启动、指定步骤或续做，按需选择文档与验证 | 复制到 Codex task |
| [GLM worker task](./glm-worker-task.md) | GPT-6 派发单个开发切片、接收执行证据并独立验收 | owning Tracker 内任务单；进程输入为临时副本 |
...
`closeout.md`。确有独有终态证据时，直接在 `docs/archive/<year>/` 创建一份紧凑证据，
并由当前 authority 入链。
