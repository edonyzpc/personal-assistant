...
> [Step 3 SDD](./operations-agent-step3-sdd.md).
>
...
> - **What does NOT live here**: Write Action Framework v1 的基础设施实现细节（→ `docs/architecture/write-action-framework-sdd.md`）、其他未来 action 家族（replace-section / multi-file / command / shell）的实现设计、Pagelet review note 创建流程（→ `docs/archive/review-assistant-sdd.md`）、Skill 系统扩展（→ SPEC-C2）。
...
| Implementation Status | **Superseded; bounded Step 2/3 runtime is build-available and remains disabled per vault by default** |
| 对应版本 | Historical SPEC-C1; replaced by B-101 Step 2/3 |
| 决策依据 | Current [Operations capability](./agent-operations-capability.md)、[Step 2 SDD](./operations-agent-step2-sdd.md) 与 [Step 3 SDD](./operations-agent-step3-sdd.md)；本文件仅保留历史设计背景 |
| 二层命名层级 | `Operations Agent mode (本 SDD)` ⟶ `Write Action Framework v1 (基础设施层)` ⟶ `append-to-current-note (首个 mode-level action family)` |
| 历史前置条件 | Write Action Framework v1 至少 8 周实战验证且无 security issue；Orchestrator 分解完成（SPEC-B5）；Operations Agent mode SDD 审查通过 |
| 历史阻塞条件 | 已由当前四工具、prompt/Settings、确认、安全与 Obsidian dogfood 边界取代；不授权更广能力 |
| 主作者 | PA core |
| 上次更新 | 2026-06-16 |
...
从只读到读写的演进是产品路线图的核心方向之一（`docs/archive/write-action-design-handoff.md` 明确"first candidate write action"为 `append answer to current note`）。
...
> **来源**：`docs/development/proposals/operations-agent/operations-agent-plan.md` §Scope、`docs/archive/write-action-design-handoff.md` §Candidate Action Families、`docs/archive/v2-post-release-spec-driven-development.md` SPEC-C1，以及当前 `docs/development-roadmap.md` / `docs/backlog.md`。
...
| 来源 | 本 SDD 章节 |
|------|------------|
| `docs/development/proposals/operations-agent/operations-agent-plan.md` §Scope | §1.3、§2 |
| `docs/development/proposals/operations-agent/operations-agent-plan.md` §Preview And Confirmation | §3.2.4、§4.1、§4.3 |
| `docs/development/proposals/operations-agent/operations-agent-plan.md` §Target Confinement | §3.2.3、§4.2 |
| `docs/development/proposals/operations-agent/operations-agent-plan.md` §Rollback And Failure | §3.2.6 |
| `docs/development/proposals/operations-agent/operations-agent-plan.md` §Audit And Privacy | §3.4（debug emit 延续，不新增 audit） |
| `docs/development/proposals/operations-agent/operations-agent-plan.md` §Test Gates | §5 |
| `docs/archive/write-action-design-handoff.md` §Candidate Action Families | §1.3、§3.2.1 |
| `docs/archive/write-action-design-handoff.md` §Trust Model | §3.4、§4.5 |
| `docs/archive/write-action-design-handoff.md` §Required Gates | §3.2.2 |
| `docs/archive/write-action-design-handoff.md` §Preview Contract | §3.2.4 |
| `docs/archive/write-action-design-handoff.md` §Runtime Boundary | §3.3 |
| `docs/archive/write-action-design-handoff.md` §Minimal Implementation Sequence | §8 |
| `docs/architecture/write-action-framework-sdd.md` §1.1 二层命名 | §0 |
| `docs/architecture/write-action-framework-sdd.md` §1.3 4-gate 流向 | §3.2.2 |
| `docs/architecture/write-action-framework-sdd.md` §2.1 Preview-Confirmation | §3.2.4 |
| `docs/architecture/write-action-framework-sdd.md` §2.2 Target Confinement | §3.2.3 |
| `docs/architecture/write-action-framework-sdd.md` §2.3 Stale Re-read (mode B 预留) | §3.2.5 |
| `docs/architecture/write-action-framework-sdd.md` §4 PolicyEngine | §3.3 |
| `docs/architecture/write-action-framework-sdd.md` §5.3 Self-Write Set | §7.2 |
| `docs/architecture/write-action-framework-sdd.md` §10 Open Decisions | §3.2.5、§9 |
| `docs/archive/v2-post-release-spec-driven-development.md` SPEC-C1; current `docs/development-roadmap.md` / `docs/backlog.md` | §0、§2、§8 |
| `docs/backlog.md` §Future Milestones (write action gate) | §0 |
| Memory `project_action_mode_roadmap` | §0（命名层级）、§3.3（PolicyEngine 扩展方向） |

---

> 文档结束。SPEC-C1 的 append-to-current-note 基础实现已存在，但当前 `2.8.0` 仍通过 `OPERATIONS_AGENT_RUNTIME_ENABLED=false` 禁止暴露；后续恢复产品化时需同步更新当前 `docs/backlog.md`、`docs/development-roadmap.md`，并在需要追溯历史 ledger 时参考 `docs/archive/v2-post-release-spec-driven-development.md`。
