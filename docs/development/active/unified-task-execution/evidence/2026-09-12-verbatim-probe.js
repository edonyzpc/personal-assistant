if (window.__b135Verbatim?.state === 'running') throw new Error('Probe already running');
window.__b135Verbatim = { state: 'running', startedAt: new Date().toISOString(), results: [] };
(async () => {
    const result = window.__b135Verbatim;
    ...
    result.config = { provider: plugin.settings.aiProvider, model: plugin.settings.chatModelName,
    ...
    for (const entry of [{ name: 'original-native', prompt: original, native: true },
        { name: 'delimited-native', prompt: delimited, native: true },
        { name: 'delimited-legacy', prompt: delimited, native: false }]) {
        ...
        const host = { app: { vault, workspace: { getActiveFile: () => null },
            ...
            settings: { ...plugin.settings, debug: false, memoryEnabled: false, webSearchEnabled: false,
        ...
        service.aiUtils.createChatModel = async (temperature, options = {}) => {
            ...
            model._streamResponseChunks = async function* (messages, callOptions, manager) {
                const capture = { messages: messages.map(message => ({ role: message._getType(), content: message.content })), chunks: [] };
    ...
    result.state = 'done';
})().catch(error => { window.__b135Verbatim.state = 'failed'; window.__b135Verbatim.error = String(error); });
'started; poll window.__b135Verbatim';
