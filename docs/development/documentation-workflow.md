...
`docs/` 是持久产品工作的 repo-local system of record。随口 PA idea 默认只停留在当前对话：它尚未被要求持久保存，也未进入版本候选、产品决策、跨会话研究或执行，因此不分配 `B-xxx`。用户明确要求记录/保存，或触发任一晋升条件时，才建立最小 repo-local Backlog/Discovery/Decision/Spec 入口；Chat、Issue、邮件或本机 Memory 只能提供输入，不能替代仓库内状态。
...
ID、指定文档路径或维护状态；Agent 负责查重、建档、校准、验证与归档，只在
产品价值、范围、优先级、风险接受或实现/Git/release 授权确实需要用户判断时，
一次提出一个问题。

自然语言边界固定为：

- “有个想法/随口分享”默认只在当前对话讨论，不创建 `B-xxx`，不自动开发。
- “记录/保存这个 idea”表示持久化意图：查重后创建或复用一个最小 Backlog 条目，不自动创建 Discovery/Spec/Active Package，也不自动开发。
...
该 skill 负责持久化与晋升后的文档路由；复杂实现继续复用 `sdd-lifecycle`。
项目不使用外部 tracker 作为默认 idea inbox、规划镜像或同步 gate。既有外部链接只
作为历史来源，不授权后续读取或写入。以下规则仍是 Agent 的执行标准，不是要求用户
记忆的操作手册。
...
| L1 Discovery / Decision | 已明确持久记录或晋升，需要产品决策、版本候选、跨会话研究或方案比较 | Backlog ID；需要跨会话讨论时创建 Discovery Brief；重要结论创建 Decision Record | Accepted 后进入 Spec；Deferred 回 Backlog；Rejected/Cancelled 归档或删除 |
...
只有新增或改变 PA runtime、用户行为、数据/权限/隐私、Obsidian UI 或产品生命周期时，才进入 L2/L3 Product Decision + Product Spec 链。纯 repo governance/tooling 即使跨多个文档、Skill、checker 或 CI，也使用 L2G；若实施中触碰上述产品边界，必须停止 L2G 并升级到 L2/L3。用户可以直接授权 engineering bootstrap 并分配 `B-xxx`，此时记录 `User direct authorization`，不要求外部 tracker 来源。
...
```mermaid
flowchart LR
  Raw["Raw idea / current conversation"] -->|explicit record or promotion trigger| Capture
...
上图的 Raw → Decision → Product Spec 是产品链；L2G 可由用户直接 engineering authorization 建立 `B-xxx` + Current Governance Contract 后进入 Planned，不需要外部 intake，也不经过 Product Decision/Spec。
...
Backlog 接收用户明确要求持久记录，或已经晋升的 product work，不承担每个随口 idea 的自动收件箱。创建 `B-xxx` 前先搜索重复项；明确记录/保存，或事项需要产品决策、进入版本候选、跨会话研究或执行时，分配稳定 ID 写入 [Backlog](../backlog.md)。一条 Backlog 至少包含：问题/结果、当前边界、下一步或启动条件、来源/依据；来源可以是 `User request YYYY-MM-DD`，不需要为了制造链接而提前创建 Discovery。

- 随口 PA idea 留在当前对话，不创建 `B-xxx`；除非用户明确要求记录/保存，否则不做 repo 或外部写入。
- 明确“记录/保存”本身就是持久化 gate；创建或复用最小 Backlog 行，但不自动升级成 Discovery、Spec 或 Active Package。
- 晋升触发器只有四类：需要产品决策、进入版本候选、需要跨会话研究、或准备进入执行。任一触发后必须先建立 repo-local Backlog authority。
- 当次即可完成且不改变契约的 L0 修复，不必创建 Backlog。
- 既有外部 Issue/link 只作为历史来源，不承担状态镜像；影响范围、优先级、决定或实现的结论只写 repo docs。
...
按任务选择最短路径，不要求每次顺序读完整条链：

1. 先用稳定 ID、feature slug 或产品概念搜索，并检查 `git status` / focused diff。
2. 记录或状态查询只读 [Backlog](../backlog.md) 与匹配的当前 index/Tracker；路由不清时才读 [docs index](../index.md)。
3. 产品行为只读匹配的 Decision/Product Spec，必要时补 North Star；repo governance 只读匹配的 Governance Contract。
4. 只有进入执行阶段才读 Feature Home、Plan、SDD、Tracker 与相关 Architecture/code/tests。
5. 只有创建新 artifact 时读对应模板；只有当前权威链接历史原因或 closeout preflight 时进入 [Archive](../archive/README.md)。

更新同样遵循最小写入：只更新语义或状态真实变化的权威文档，以及 checker 强制的一处必要索引/derived mirror；不要为了“同步”改写无变化的 Roadmap、Decision、Spec 或过程日志。
