...
### 决策 14A: User Profile 与工具策略边界

**结论：User Profile 是软上下文，不是工具开关**

Type A 可以记录用户明确表达的长期偏好，用于 AI Insights 展示和回答风格个性化；但这些偏好不能直接注册、禁用或强制调用工具。尤其是 WebSearch：是否联网搜索应由当前用户输入意图、显式 source constraint、设置状态和 PA Agent host policy 决定。

**决策理由：** `User Profile` 是从对话中提取的自然语言画像，存在误提取、过期和上下文丢失风险。如果把它当作可执行工具策略，单次污染就可能长期禁用 WebSearch、Memory 或 current-note 能力。当前产品选择是：`User Profile` 保持软上下文；用户当轮说"不要联网"时只影响当轮；用户说"以后不要用 web search"这类长期偏好可以被记录和展示，但不会自动变成运行时硬约束。若未来需要持久工具控制，应新增结构化 `ToolPolicy` / `AgentControlSnapshot` 路径，并提供明确的 UI、优先级和撤销机制。
