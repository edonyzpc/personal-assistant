...
> Status update 2026-06-16: SPEC-D6~D8 implementation is complete in the main worktree; broad checks, deploy, and Obsidian smoke evidence are recorded in the SDD tracker.
...
| Task | Files | Effort |
|------|-------|--------|
| Micro-compaction（预算驱动 + 2 轮保护） | `src/ai-services/context/PaAgentContextCompactor.ts` | 3-4d |
| Full compaction（history summary） | `src/ai-services/context/PaAgentContextCompactor.ts` | 2-3d |
| Context budget 机制（动态分配 + 预算监控） | `pa-agent-runtime.ts`, `src/ai-services/context/PaAgentContextBudget.ts` | 2-3d |

#### WT-A: `feat/type-a-profile` (SPEC-D7, ~3-5d)

| Task | Files | Effort |
|------|-------|--------|
| 用户画像自动提取（纯自动模式） | `src/ai-services/memory-extraction/type-a-extractor.ts` | 1.5-2d |
| 提取触发（定时 + 对话边界） | `chat-view.ts`, `plugin.ts`, `extraction-scheduler.ts` | 1d |
| 画像存储（vault-scoped IndexedDB） | `profile-store.ts` | 0.5-1d |
...
| Task | Files | Effort |
|------|-------|--------|
| 6 维度元认知（主题/标签/链接/习惯/空白/趋势） | `src/ai-services/memory-extraction/type-c-analyzer.ts` | 5-7d |
| Extraction pipeline（独立调度,与 Type A 分离） | `extraction-scheduler.ts` | 1-2d |
| Vault insights 存储（插件内部 snapshot，默认不写 vault） | scheduler snapshot | 1d |
