...
export interface RetrievalDiagnosticEventInput {
    ...
    runId?: string;
    /** Run-local Memory invocation order; content-free and never user supplied. */
...
function normalizeEvent(
    ...
    const reason = typeof input.reason === "string" && REASONS.has(input.reason)
        ? input.reason
        : undefined;
    const invocationScoped = input.phase !== "finalization_reserve"
        || (input.outcome === "skipped" && input.reason === "reserve_protected");
    const invocationOrdinal = invocationScoped
        && typeof input.invocationOrdinal === "number"
        && Number.isSafeInteger(input.invocationOrdinal)
        && input.invocationOrdinal >= 0
