...
const RERANK_SYSTEM_PROMPT = [
    ...
    "- relevant requires a non-empty ranking and needsMoreEvidence=false",
    "- partially_relevant requires a non-empty ranking and an explicit needsMoreEvidence boolean",
    ...
    'Return ONLY valid JSON: {"verdict":"relevant|partially_relevant|none_relevant","ranking":[...],"needsMoreEvidence":false}.',
].join("\n");
...
export class MemorySearchTool {
    ...
    private async searchVss(
        ...
        const unboundRecord = this.host.createRetrievalDiagnosticRecorder
            ...
            : this.host.recordRetrievalDiagnostic
                ...
                : undefined;
        const record = unboundRecord && invocation?.runEpoch
            ? (event: RetrievalDiagnosticEventInput) => unboundRecord({
                ...event,
                runId: invocation.runEpoch,
            })
            : unboundRecord;
    ...
    private async rerankCandidates(
        ...
        const prepared = await this.prepareReranker(selectedModel, signal, absoluteDeadlineMs);
    ...
    private async collectGraphCandidates(input: {
        ...
        try {
            ...
            if (shouldRecordGraphWorkerFailureTerminal(reason, cancelRequestedInRequest)) {
                safeRecordDiagnostic(input.record, {
                    phase: "graph_worker",
...
export function shouldRecordGraphWorkerFailureTerminal(
    ...
    return reason !== "graph-rank-aborted" || cancelRequestedInRequest <= 0;
}
