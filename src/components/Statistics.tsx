...
const Statistics = ({ app, plugin, staticsFileData }: Props) => {
	const statics = useMemo(
		() => JSON.parse(staticsFileData),
		[staticsFileData]
	);
	const history: string[] = useMemo(
		() => Object.keys(statics.history),
		[statics.history]
	);
	...
	const { wordsData, pageData, filesData, pagesData } = useMemo(() => {
		...
		for (const date of history) {
			...
			files.push({
				key: date,
				value: Number(statics.history[date].files),
			});
			totalPages.push({
				key: date,
				value: Number(statics.history[date].totalPages),
			});
		}
		return {
			wordsData: words,
	...
	const animation = useMemo<ChartOptions<"line">["animations"]>(() => {
		if (plugin.settings.animation) {
			const totalDuration = 10000;
			const delayBetweenPoints =
				totalDuration / Math.max(history.length, 1);
	...
	const dataDaily = useMemo<ChartData<"line", CustomChartData[]>>(
		() => ({
			datasets: [
				{
					label: "Daily Words",
	...
	const dataTotal = useMemo<ChartData<"line", CustomChartData[]>>(
		() => ({
			datasets: [
				{
					label: "Total Files",
	...
	const optionsDaily = useMemo<ChartOptions<"line">>(
		() => ({
			responsive: true,
	...
	const optionsTotal = useMemo<ChartOptions<"line">>(
		() => ({
		...
		>
			...
			>
				{isTotal ? (
					...
					<LineChart data={dataDaily} options={optionsDaily} />
				)}
			</Suspense>
		</div>
	);
