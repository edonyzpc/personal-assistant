...
<script lang="ts">
	...
	function determineEmbedType(node: Element): EmbedType {
		...
		for (const [embedTypeKey, embedTypeRegex] of Object.entries(
			embedTypeToSrcRegex,
		)) {
	...
	function getClosestMatchingFilePath(
		...
		allMatches.sort((left, right) => {
			if (
				left.startsWith(containingDir) &&
				!right.startsWith(containingDir)
			) {
				return -1;
			}
			if (
	...
	const getMediaUri = (
		...
		const matchingPath = getClosestMatchingFilePath(
			vault,
			mediaSrc,
			containingNotePath,
		);
	...
	const renderMarkdown = async (
		...
		let { contentStart } = getFrontMatterInfo(markdown);
		let contentWithoutFrontmatter = markdown.slice(contentStart);
			...
			switch (embedType) {
				...
				case EmbedType.PDF:
					if (!Platform.isDesktop) {
						plugin.log(
							"Embedded PDFs are only supported on the desktop.",
						);
						return;
					}
				...
				default:
					plugin.log(
						"Could not determine embedding type for element:",
					);
					plugin.log(node);
			}
		});
...
<div class="markdown-reading-view" style="width: 100%; height: 100%;">
	<div
		class="markdown-preview-view markdown-rendered node-insert-event is-readable-line-width allow-fold-headings show-indentation-guide allow-fold-lists show-properties"
		tabindex="-1"
		style="tab-size: 4;"
	>
		<div class="markdown-preview-sizer markdown-preview-section">
			<div
				class="markdown-preview-pusher"
			...
			<div class="recordlist-wrapper" id="persoanl-assistant-record-list">
				{#each fileNames as fileName, idx}
					...
					{#await readMarkdownFile(fileName) then fileString}
						...
						{#await renderMarkdown(fileString, subContainer(`record-wrapper-sub-${idx}`), fileName, plugin)}{/await}
					{/await}
				{/each}
			</div>
		{:else}
		...
		{#each fileNames as fileName, idx}
			<!-- svelte-ignore empty-block -->
			{#await addClickableforRecord(`record-wrapper-sub-${idx}`, fileName)}{/await}
		{/each}
	</div>
</div>
