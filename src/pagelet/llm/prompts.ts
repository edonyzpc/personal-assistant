...
export function buildQuickReviewPrompt(
    ...
    const systemPrompt = [
        SYSTEM_PROMPT_BASE,
        "",
        "SCENARIO: Quick review of notes.",
        "- Review the notes for clarity, completeness, and opportunities.",
        "- Produce up to 5 findings, each categorized as insight/action/connection/gap.",
    ...
    const userPrompt = [
        `Review these ${noteContents.length} note(s) and produce findings with categories:`,
        "",
        formatNotes(truncated),
        "",
        "Produce the JSON output now.",
...
/**
 * **Writing Assist**: analyze the current note and suggest connections to
 * past notes or improvements to the writing.
 */
export function buildWritingAssistPrompt(
    currentNote: { path: string; content: string },
    budget: { input: number; output: number },
): PromptBuildResult {
    const systemOverhead = 400;
    const available = Math.max(0, budget.input - systemOverhead);
    const truncatedContent = truncateToTokenBudget(currentNote.content, available);

    const systemPrompt = [
        SYSTEM_PROMPT_BASE,
        "",
        "SCENARIO: Writing assistance for a single note.",
        "- Analyze the note the user is currently editing.",
        "- Suggest improvements: gaps to fill, points to clarify, possible connections to other topics.",
        "- Produce up to 4 findings.",
        "- Be constructive and specific; reference exact sections when possible.",
        "",
        STRUCTURED_OUTPUT_SCHEMA,
    ].join("\n");

    const userPrompt = [
        "Analyze the following note and suggest improvements or connections:",
        "",
        `--- ${currentNote.path} ---`,
        truncatedContent,
        "",
...
export function buildDiscoveryPrompt(
    ...
    const systemPrompt = [
        ...
        '- Use category "connection" for thematic links, "gap" for missing bridges, "insight" for surprising overlaps.',
