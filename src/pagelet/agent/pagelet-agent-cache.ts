...
/**
 * A validated cache read that has not yet published a positive regrouping.
 * The mutation version is an opaque compare-and-commit token: callers must
 * commit it synchronously after their own final lifecycle/policy fence.
 */
export interface PageletAgentPreparedCacheRead {
    readonly anchorPath: string;
    readonly collection: PageletAgentVerifiedInsightCollection;
    readonly evidenceEpoch: string | undefined;
    readonly mutationVersion: number;
    readonly needsRegroup: boolean;
}
...
export class PageletAgentCache {
    ...
    async getValidCollection(
        ...
        const prepared = await this.prepareValidCollection(options);
        throwIfAborted(options.signal);
        if (!prepared) return null;
        const epochAtCommit = readEvidenceEpoch(options.getEvidenceEpoch);
        if (
            !epochAtCommit.ok
            || epochAtCommit.value !== prepared.evidenceEpoch
            || !preparedCollectionPathsAllowed(prepared.collection, options.isPathAllowed)
        ) return null;
        return this.commitPreparedCollection(prepared);
    }

    async prepareValidCollection(
        options: PageletAgentCacheReadOptions,
    ): Promise<PageletAgentPreparedCacheRead | null> {
        ...
        let stableEvidenceEpoch: string | undefined;
        for (let attempt = 0; attempt < attempts; attempt += 1) {
            ...
            if (epochAfter.ok && epochAfter.value === epochBefore.value) {
                ...
                stableEvidenceEpoch = epochAfter.value;
                break;
            }
        }
        ...
        if (validInsights.length === 0) {
            if (
                this.mutationVersion === mutationVersionAtStart
                && this.entries.get(options.anchor.path) === entry
            ) this.deleteEntry(options.anchor.path);
            return null;
        }
        const needsRegroup = validInsights.length !== entry.collection.insights.length;
        const collection = needsRegroup
            ? regroupCollection(entry.collection, validInsights)
            : entry.collection;
        if (
            this.mutationVersion !== mutationVersionAtStart
            || this.entries.get(options.anchor.path) !== entry
        ...
        return Object.freeze({
            anchorPath: options.anchor.path,
            collection: cloneCollection(collection),
            evidenceEpoch: stableEvidenceEpoch,
            mutationVersion: mutationVersionAtStart,
            needsRegroup,
        });
    }
    ...
    ): PageletAgentVerifiedInsightCollection | null {
        if (
            this.mutationVersion !== prepared.mutationVersion
            || !this.entries.has(prepared.anchorPath)
        ) return null;
        if (prepared.needsRegroup) {
            const versionBeforeCommit = this.mutationVersion;
            this.putCollection(prepared.collection);
            if (this.mutationVersion !== versionBeforeCommit + 1) return null;
        }
        const committed = this.entries.get(prepared.anchorPath);
        return committed ? cloneCollection(committed.collection) : null;
    }

    deletePreparedCollection(prepared: PageletAgentPreparedCacheRead): void {
        if (this.mutationVersion !== prepared.mutationVersion) return;
        this.deleteEntry(prepared.anchorPath);
    }
...
export function pageletAgentPolicyIdentityKey(identity: PageletAgentPolicyIdentity): string {
    return stableStringify({
        dataBoundaryIdentity: identity.dataBoundaryIdentity,
        providerPolicyIdentity: identity.providerPolicyIdentity,
        modelIdentity: identity.modelIdentity,
        locale: normalizeLocale(identity.locale),
    });
}
...
function preparedCollectionPathsAllowed(
    collection: PageletAgentVerifiedInsightCollection,
    predicate: (path: string) => boolean,
): boolean {
    if (!safePathAllowed(predicate, collection.anchor.path)) return false;
    return collection.insights.every((insight) => (
        insight.sources.every((source) => safePathAllowed(predicate, source.path))
    ));
}
